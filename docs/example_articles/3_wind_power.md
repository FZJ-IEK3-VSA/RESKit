...
in the ETHOS.Data catalogue is `rk.wind.turbine_library(data.paths("turbine_library")["turbines"])`.
Custom baseline turbine definitions are passed to `OnshoreParameters(fp=...)` and
`OffshoreParameters(fp=...)`. See
[Get input data from the ETHOS.Data catalogue](../how_to/get_input_data.md#location-rasters-and-the-turbine-library).
