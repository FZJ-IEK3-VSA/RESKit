...
cmap = LinearSegmentedColormap.from_list("peter", [cmap(20), cmap(100)], N=len(tes),)

for i, tes_i in enumerate(tes):

    data_i = data[:, i]

    ax.plot(sm, data_i, label=f"TES: {tes_i}h", color=cmap(i))
