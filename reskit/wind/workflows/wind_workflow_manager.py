    ...
    def simulate(
        ...
        for _batch in range(int(_batches)):
            ...
            if (_target_cfs > 1).any():
                raise ValueError(
                    f"The current correction factors lead to average target capacity factors greater 1.0."
                )
            ...
            while (abs(_deviations - 1) > tolerance).any():
                ...
                _diverging = (abs(_deviations_new) > abs(_deviations))
                # make sure divergence occurs only at very low cfs (due to effects of cut-in windspeed)
                assert all((_diverging*avg_gen)<0.05), \
                    f"Diverging placements with avg. cf > 0.05 found: {(_diverging*avg_gen)[(_diverging*avg_gen)>0.05]}"
                del avg_gen, _deviations_new # RAM
                _diverging *= _deviations # set deviation values only for diverging locations
                _diverging[_diverging == 0] = 1 # set other location deviations to 1, only here
