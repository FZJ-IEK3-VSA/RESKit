...
def offshore_bos(cp, rd, hh, depth, distance_to_shore, distance_to_bus, foundation, mooring_count, anchor, turbine_count, turbine_spacing, turbine_row_spacing):
    ...
    if foundation == 'monopile':
        ...
        monopileMass = (np.power((cp * 1000), 1.5) + (np.power(hh, 3.7) / 10) +
                        2100 * np.power(depth, 2.25) + np.power((RNAMass * 1000), 1.13)) / 10000
        ...
        jacketTPMass = 1 / (((-0.0131 + 0.0381) / np.log(cp)) -
                            0.00000000227 * np.power(depth, 3))
        ...
        sparSCMass = 535.93 + 17.664 * \
            np.power(cp, 2) + 0.02328 * depth * np.log(depth)
        ...
        semiSubmersibleHPMass = -0.4397 * \
            np.power(cp, 2) + 21.145 * cp + 177.42
        semiSubmersibleHPCost = semiSubmersibleHPMass * semiSubmersibleHPCostRate

        foundationCost = semiSubmersibleSCCost + \
            semiSubmersibleTCost + semiSubmersibleHPCost
    ...
    if fixedType:
        ...
        secondarySteelSubstructureMass = -0.153 * \
            np.power(cp, 2) + 6.54 * cp + 128.34
    ...
    singleStringPower1 = np.sqrt(
        3) * cable1CurrentRating * arrayVoltage * powerFactor / 1000
    singleStringPower2 = np.sqrt(
        3) * cable2CurrentRating * arrayVoltage * powerFactor / 1000
    ...
    numberofTurbineInterfacesPerArrayCable1 = numberofTurbinesperArrayCable1 * \
        numberofStrings * 2
    ...
    numberofTurbineInterfacesPerArrayCable2 = (
        max1_Cable1 * numberofStrings + max2_Cable1) * 2

    numberofArrayCableSubstationInterfaces = numberofStrings

    if fixedType:
        arrayCable1Length = (turbine_spacing * rd + depth * 2) * (
            numberofTurbineInterfacesPerArrayCable1 / 2) * (1 + excessCableFactor)
        ...
        freeHangingCableLength = (
            depth / np.cos(systemAngle * np.pi / 180) * (catenaryLengthFactor + 1)) + 190

        fixedCableLength = (turbine_spacing * rd) - \
            (2 * np.tan(systemAngle * np.pi / 180) * depth) - 70

        arrayCable1Length = (2 * freeHangingCableLength) * (
            numberofTurbineInterfacesPerArrayCable1 / 2) * (1 + excessCableFactor)
        arrayCable1Length /= 1000  # convert to km
    ...
    singleExportCablePower = np.sqrt(
        3) * cable2CurrentRating * arrayVoltage * powerFactor / 1000
    numberOfExportCables = np.floor_divide(
        cp * turbine_count, singleExportCablePower) + 1

    if fixedType:
        exportCableLength = (distance_to_shore * 1000 +
                             depth) * numberOfExportCables * 1.1
        exportCableLength /= 1000  # convert to km

        exportCableandAncillaryCost = exportCableLength * \
            externalCableCost + numberOfExportCables * substationInterfaceCost
    else:
        exportCableLength = (distance_to_shore * 1000 +
                             freeHangingCableLength + 500) * numberOfExportCables * 1.1
        exportCableLength /= 1000  # convert to km
    ...
    numberOfMainPowerTransformers = np.floor_divide(
        turbine_count * cp, 250) + 1

    # equation 72 in [1] is simplified
    singleMPTRating = np.round(
        turbine_count * cp * 1.15 / numberOfMainPowerTransformers, -1)

    mainPowerTransformerCost = numberOfMainPowerTransformers * \
        singleMPTRating * mainPowerTransformerCostRate

    switchgearCost = numberOfMainPowerTransformers * \
        (highVoltageSwitchgearCost + mediumVoltageSwitchgearCost)

    shuntReactorCost = singleMPTRating * \
        numberOfMainPowerTransformers * shuntReactorCostRate * 0.5

    ancillarySystemsCost = dieselGeneratorBackupCost + \
        workspaceCost + otherAncillaryCosts

    offshoreSubstationTopsideMass = 3.85 * \
        (singleMPTRating * numberOfMainPowerTransformers) + 285
    offshoreSubstationTopsideCost = offshoreSubstationTopsideMass * \
        fabricationCostRate + topsideDesignCost
    assemblyFactor = 1  # could not find a number...

    offshoreSubstationTopsideLandAssemblyCost = (
        switchgearCost + shuntReactorCost + mainPowerTransformerCost) * assemblyFactor

    if fixedType:
        offshoreSubstationSubstructureMass = 0.4 * offshoreSubstationTopsideMass

        substationSubstructurePileMass = 8 * \
            np.power(offshoreSubstationSubstructureMass, 0.5574)
        ...
        if foundation == 'spar':  # WHY WAS IT SPAR BEFORE? WE ARE DOING THINGS WITH SEMISUBMERSIBLE
            ...
            semiSubmersibleSCMass = -0.9571 * \
                np.power(cp, 2) + 40.89 * cp + 802.09
            semiSubmersibleSCCost = semiSubmersibleSCMass * semiSubmersibleSCCostRate

            # semiSubmersible truss mass is called as semiSubmersibleTMass
            semiSubmersibleTMass = 2.7894 * \
                np.power(cp, 2) + 15.591 * cp + 266.03
            semiSubmersibleTCost = semiSubmersibleTMass * semiSubmersibleTCostRate
        ...
        semiSubmersibleMass = semiSubmersibleSCMass + \
            semiSubmersibleTMass + semiSubmersibleHPMass
    ...
    onshoreSubstationCost = 11652 * \
        (interconnectVoltage + cp * turbine_count) + 1200000

    onshoreSubstationMiscCost = 11795 * \
        np.power(cp * turbine_count, 0.3549) + 350000
    ...
    assemblyAndInstallationCost = np.ones(
        totalElectricalInfrastructureCosts.shape)
