...
def visibility_from_topography(lon, lat, elevation_raster, base_elevation=None, eye_level=2, max_degree=0.1, degree_step=0.003, theta_step=3, _interpolation='cubic',):
    ...
    * You can plot the outputs nicely with:
    ...
    approx_center_earth_thetas = np.arange(
        0, max_degree+degree_step, degree_step)+degree_step/2
    approx_center_earth_thetas_r = np.radians(approx_center_earth_thetas)

    if base_elevation is None:
        base_elevation = gk.raster.interpolateValues(
            elevation_raster, (lon, lat)) + eye_level
    ...
    BdotC = np.sin(lat_r)*np.sin(sample_lats_r)*np.cos(lon_r -
                                                       sample_lons_r) + np.cos(lat_r)*np.cos(sample_lats_r)
    center_earth_theta = np.arccos(BdotC)

    locs = np.column_stack([sample_lons.flatten(), sample_lats.flatten(), ])
...
#     elevs = np.full( locs.shape[0], base_elevation-eye_level)
    ...
    for di in range(1, len(approx_center_earth_thetas_r)):
        visibility.append(planar_angle[di, :] >= (
            planar_angle[:di, :].max(axis=0)))
...
if __name__ == "__main__":
    ...
    fig = plt.figure()
    ax = fig.add_subplot(111, polar=True)
    h = ax.pcolormesh(a['elevation_angle'].index,
    ...
    fig = plt.figure()
    ax = fig.add_subplot(111, polar=True)
    h = ax.pcolormesh(a['ground_visibility'].index,
                      a['ground_visibility'].columns,
                      a['ground_visibility'].values.T)
    plt.colorbar(h)
    plt.show()

    print(a['ground_visibility'].sum().sum() /
