...
def get_tile_XY(zoom, lon=None, lat=None, geom=None):
    ...
    if geom is not None:
        ...
        if not (isinstance(geom, osgeo.ogr.Geometry) and "POINT" in geom.GetGeometryName()):
            raise ValueError(
                f"geom must be an osgeo.ogr.Geometry point geometry")
        assert geom.GetSpatialReference() is None or geom.GetSpatialReference() == gk.srs.loadSRS(4326),\
...
def get_dataframe_with_weather_tilepaths(placements, weather_path, zoom):
    ...
    if not isinstance(placements, pd.DataFrame):
        ...
        if all([isinstance(x, tuple) and all([isinstance(latlon, (float, int)) for latlon in x]) for x in placements]):
            ...
            assert all([x.GetSpatialReference() is None or x.GetSpatialReference() == gk.srs.loadSRS(4326) for x in placements]),\
                f"All srs of objects in placements must be EPDG:4326"
            assert all(["POINT" in x.GetGeometryName() for x in placements]),\
    ...
    if weather_path is None:
        ...
        assert 'source' in placements.columns, f"weather_path is None yet no 'source' attribute in placements dataframe."
        if any([_str in _fp for _str in ['<X-TILE>', '<Y-TILE>', '<ZOOM>'] for _fp in placements.source]):
            ...
            placements['source'] = placements.apply(lambda x: _get_tilepath(
                weather_path=x.source, zoom=zoom, lon=x.lon, lat=x.lat), axis=1)
    else:
        # make sure we have no source column to avoid overwriting data
        assert not 'source' in placements.columns, f"If weather_path is given, placements must not have a 'source' attribute already"

        def _get_tilepath(weather_path, zoom, lat, lon):
            if '<X-TILE>' in weather_path or '<Y-TILE>' in weather_path:
                assert isinstance(
                    zoom, int), f"zoom must be a positive integer tiling level if weather_path contains X/Y spacers"
                _X, _Y = get_tile_XY(zoom=zoom, lon=lon, lat=lat, geom=None)
                return weather_path.replace('<X-TILE>', str(_X)).replace('<Y-TILE>', str(_Y))
            else:
                return weather_path
        # add source column with the actual tile filepaths
        placements['source'] = placements.apply(lambda x: _get_tilepath(
            weather_path=weather_path, zoom=zoom, lon=x.lon, lat=x.lat), axis=1)
