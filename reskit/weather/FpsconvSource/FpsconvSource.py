...
class FpsconvSource(NCSource):
    """
    SChen: The FpsconvSource, which is a convection-permitting scale regional climate projection data from CORDEX (the Coordinated Regional Climate Downscaling Experiment)
    ...
    These constants include:
        MAX_LON_DIFFERENCE = 0.0276 # SChen this is for 0.0275 FPSCONV grid
            The maximum longitude difference to accept between a grid cell's center and the coordinates
                to extract data for

        MAX_LAT_DIFFERENCE = 0.0276 # SChen this is for 0.0275 FPSCONV grid
        ...
        WIND_SPEED_HEIGHT_FOR_SOLAR_ENERGY = 10
            The suggested altitude of wind speed data to use for solar-energy simulations
    ...
    reskit.weather.IconlamconvSource
    """
    ...
    MAX_LON_DIFFERENCE = 0.0276  # 0.034  # 0.26 SChen
    MAX_LAT_DIFFERENCE = 0.0276  # 0.034  # 0.26 SChen

    def __init__(self, source, bounds=None, index_pad=5, **kwargs):
        """Initialize a regular lat-lon grid style netCDF4 file source
    ...
    loc_to_index = NCSource._loc_to_index_rect(0.0275, 0.0275)  # (0.033, 0.033)  # (0.25, 0.25) SChen
    ...
    def sload_boundary_layer_height(self):
        ...
        """
        return self.load("zmla", "boundary_layer_height")

    def sload_elevated_wind_speed(self):
        """Standard loader function for the variable 'elevated_wind_speed'

        Automatically reads the variables "ws<X>" from the given FpsconvSource source and saves
        it as the variable 'elevated_wind_speed' in the data library. In FpsconvSource, it is "ws<X>m".
        ...
        """
        return self.load("ws{}m".format(self.ELEVATED_WIND_SPEED_HEIGHT), "elevated_wind_speed")
    ...
    def sload_surface_pressure(self):
        ...
        variable 'surface_pressure' in the data library. In FpsconvSource, it is "ps".
        """
    ...
    def sload_surface_air_temperature(self):
        ...
        variable 'surface_air_temperature' in the data library. In FpsconvSource, it is "tas".

        Temperature values are also converted from kelvin to degrees celsius
        """
        return self.load("tas", name="surface_air_temperature", processor=lambda x: x - 273.15)
