    ...
    **This simply exposes the associated functions from PVLib, and is only here for convenience**
    ...
    def modules(self, group='cec'):
        ...
        if getattr(self, name) is None:
            ...
            setattr(self, name, pd.read_csv(join(DATADIR, "modules",
                                                 "sam-library-cec-modules-2017-6-5.csv"), skiprows=[1, 2], index_col=0))
        return getattr(self, name)

    def inverters(self, group='sandia'):
        name = "_" + group.lower() + "inverters"
        if getattr(self, name) is None:
            setattr(self, name, pvlib.pvsystem.retrieve_sam(
                group + "inverter").T)
        return getattr(self, name)
...
def spencerSolPos(times, lat, lon):
    ...
    d = 0.006918 - 0.399912 * np.cos(xx) + 0.07257 * np.sin(xx) - 0.006758 * np.cos(
        2 * xx) + 0.000907 * np.sin(2 * xx) - 0.006758 * np.cos(3 * xx) + 0.00907 * np.sin(3 * xx)
    #d = d*np.pi/180
    ...
    solarTime = (times.hour * 60 + times.minute + times.second / 60) + \
        9.87 * np.sin(2 * x) - 7.53 * np.cos(x) - 1.5 * np.sin(x)

    hourAngle = (solarTime / 60 - 12) * 15
    hourAngle_r = hourAngle * np.pi / 180

    zenith = np.degrees(np.arccos(np.cos(lat_r) * np.cos(d)
                                  * np.cos(hourAngle_r) + np.sin(lat_r) * np.sin(d)))
...
def myDirint(ghi, zenith, pressure, use_delta_kt_prime, temp_dew, amRel, I0):
    ...
    """
    disc_out = myDisc(ghi=ghi, zenith=zenith, I0=I0,
                      am=amRel, pressure=pressure)
...
def locToTilt(locs, convention="latitude*0.76", **k):
    ...
    **The role of this function probably needs to be readdressed since it tries to
    ...
    if convention == "ninja":
        ...
        try:
            tilt = gk.raster.interpolateValues(
                join(DATADIR, "bestTilt_europe_int.tif"), locs, **k)
        except:
            raise ResError(
                "Could not load best tilt for all locations. They may not have been included in the preprocessed data")
...
def _presim(locs, source, elev=300, module="WINAICO WSx-240P6", azimuth=180, tilt="ninja", totalSystemCapacity=None, tracking="fixed", modulesPerString=1, inverter=None, stringsPerInverter=1, rackingModel='open_rack_cell_glassback', airmassModel='kastenyoung1989', transpositionModel='perez', cellTempModel="sandia", generationModel="single-diode", inverterModel="sandia", interpolation="bilinear", loss=0.18, trackingGCR=2 / 7, trackingMaxAngle=60, frankCorrection=False, ghiScaling=None):
    ...
    if cellTempModel.lower() == "sandia":
        sandiaCellTemp = True
    elif cellTempModel.lower() == "noct":
        sandiaCellTemp = False
    else:
        raise ValueError("cellTempModel parameter not understood")

    if generationModel.lower() == "sapm":
        sandiaGenerationModel = True
    elif generationModel.lower() == "single-diode":
        sandiaGenerationModel = False
    else:
        raise ValueError("generationModel parameter not understood")

    if inverterModel.lower() == "sandia":
        sandiaInverterModel = True
    elif inverterModel.lower() == "driesse":
        sandiaInverterModel = False
    else:
        raise ValueError("generationModel parameter not understood")

    if tracking.lower() == "single-axis":
        singleAxis = True
    elif tracking.lower() == "fixed":
        singleAxis = False
    else:
        raise ValueError("tracking parameter not understood")
    ...
    if isinstance(source, NCSource):
        ...
        k = dict(locations=locs, interpolation=interpolation,
                 forceDataFrame=True, _indicies=idx)

        ghi = source.get("ghi", **k)
        if ghiScaling:
            merraAvg = gk.raster.interpolateValues(
                source.LONG_RUN_AVERAGE_GHI_SOURCE, locs, mode="linear-spline") * 24 / 1000  # make into kW/m2/day
            worldBankAvg = gk.raster.interpolateValues(ghiScaling, locs)
            scaling = worldBankAvg / merraAvg
            scaling[np.isnan(scaling)] = 0.9
            # Default scaling value defined from near-edge average in Norway, Sweden, and Finland
            scaling[locs.lats >= 59.9] = 0.9
            # Assumed to be the same as above
            scaling[locs.lats <= -55.1] = 0.9
        ...
        dni_flat = source.get(
            "dni_flat", **k) if "dni_flat" in source.data else None
    ...
    if isinstance(module, str):
        if sandiaGenerationModel:
            ...
            module = SolarLibrary.modules("sandia").loc[module]
            ...
            if isinstance(module, str):
                if module == "WINAICO WSx-240P6":
                    ...
                    # Extract module parameters
                    module = SolarLibrary.modules("cec").loc[module].copy()
    ...
    if isinstance(tilt, str):
        tilt = locToTilt(locs, tilt)
    else:
        tilt = ensureSeries(tilt, locs)
    ...
    for i, loc in enumerate(locs):
        # Only perform solor position calc for every .1 degrees
        solposLat = np.round(loc.lat, 1)
        # Only perform solor position calc for every .1 degrees
        solposLon = np.round(loc.lon, 1)
        # Only perform solor position calc for every 100 meters
        solposElev = np.round(elev[loc], -2)
        if isinstance(solposElev, pd.Series):
            raise ResError("%s is not unique" % str(loc))
    ...
    for c in ['apparent_zenith', 'azimuth', 'apparent_elevation']:
        solpos[c] = pd.DataFrame(np.column_stack(
            [_solpos[loc][c].copy() for loc in locs]), columns=locs, index=times)
    ...
    if not dni_flat is None:
        dni_flat = dni_flat[goodTimes].values
    if not dni is None:
        dni = dni[goodTimes].values
    if not dhi is None:
        dhi = dhi[goodTimes].values
    ...
    dni_extra = pvlib.irradiance.extraradiation(
        times[goodTimes], 1370, method='spencer').values
    if len(ghi.shape) > 1:
        dni_extra = np.broadcast_to(
            dni_extra.reshape((ghi.shape[0], 1)), ghi.shape)

    # Apply Frank corrections when dealing with COSMO data?
    if frankCorrection:
        ghi *= frankCorrectionFactors(ghi, dni_extra,
                                      times[goodTimes], solpos["apparent_elevation"])

    # Airmass
    amRel = np.zeros_like(ghi)
    s = solpos["apparent_zenith"] < 89
    amRel[s] = pvlib.atmosphere.relativeairmass(
        solpos["apparent_zenith"][s], model=airmassModel)
    ...
    return dict(singleAxis=singleAxis,
                ...
                tilt=tilt if isinstance(
                    tilt, np.ndarray) else tilt[locs[:]].values,
                module=module,
                azimuth=azimuth if isinstance(
                    azimuth, np.ndarray) else azimuth[locs[:]].values,
                inverter=inverter,
                moduleCap=moduleCap,
...
def my_i_from_v(resistance_shunt, resistance_series, nNsVth, voltage,
    ...
    **Should be hidden**
    ...
    argW = Rs * I0 / (a * (Rs * Gsh + 1.)) * \
        np.exp((Rs * (IL + I0) + V) / (a * (Rs * Gsh + 1.)))
...
def _simulation(singleAxis, genericSystem, tilt, module, azimuth, inverter, moduleCap, modulesPerString, stringsPerInverter, locs, times, dni, ghi, dhi, amRel, solpos, pressure, air_temp, windspeed, dni_extra, sandiaCellTemp, transpositionModel, totalSystemCapacity, sandiaGenerationModel, loss, approximateSingleDiode, goodTimes, extract=None):
    ...
    if sandiaGenerationModel:
        ...
        poa_total += poa["poa_ground_diffuse"] * pvlib.pvsystem.physicaliam(
            90 - 0.5788 * tilt + 0.002693 * np.power(tilt, 2))
        poa_total += poa["poa_sky_diffuse"] * pvlib.pvsystem.physicaliam(
            59.7 - 0.1388 * tilt + 0.001497 * np.power(tilt, 2))
        sel = poa_total > 0

        if extract == 'poa':
            df_poa_total = pd.DataFrame(
                index=times[goodTimes], columns=locs, data=poa_total)
            df_poa_total = df_poa_total.reindex(times).fillna(0)
            print('poa_total successfully returned as df')
        ...
        if approximateSingleDiode:
            ...
            if maxpoa > 2000:
                raise RuntimeError("Why is POA so huge???")
            ...
            rawDCGeneration['p_mp'] = pd.DataFrame(
                index=times[goodTimes], columns=locs)
            rawDCGeneration['p_mp'].values[sel] = interpolator(
                cellTemp[sel], poa_total[sel], grid=False)

            if not inverter is None:
                interpolator = RectBivariateSpline(
                    _temp, _poa, gen['v_mp'].reshape(poaM.shape), kx=3, ky=3)
                rawDCGeneration['v_mp'] = pd.DataFrame(
                    index=times[goodTimes], columns=locs)
                rawDCGeneration['v_mp'].values[sel] = interpolator(
                    cellTemp[sel], poa_total[sel], grid=False)
            ...
            rawDCGeneration = OrderedDict()
            for k in ["p_mp", "v_mp", ]:
                rawDCGeneration[k] = pd.DataFrame(
                    index=times[goodTimes], columns=locs)
...
def simulatePVModule(locs, source, elev=300, module="WINAICO WSx-240P6", azimuth=180, tilt="ninja", totalSystemCapacity=None, tracking="fixed", interpolation="bilinear", loss=0.18, rackingModel="open_rack_cell_glassback", approximateSingleDiode=True, extract=None, **kwargs):
    """
