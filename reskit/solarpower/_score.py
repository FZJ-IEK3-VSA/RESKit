...
def scoreOpenfieldPVLocation(ghi, roadDist, powerDist, settlementDist,
    ...
    """
    totalScore = ghiWeight \
        * linearTransition(np.array(ghi), ghiStart, ghiStop, ghiFlip)

    totalScore += roadDistWeight \
        * linearTransition(np.array(roadDist), roadDistStart,
                           roadDistStop, roadDistFlip)

    totalScore += powerDistWeight \
        * linearTransition(np.array(powerDist), powerDistStart,
                           powerDistStop, powerDistFlip)

    totalScore += settlementDistWeight \
        * linearTransition(np.array(settlementDist),
                           settlementDistStart, settlementDistStop,
                           settlementDistFlip)
