...
class PTRWorkflowManager(SolarWorkflowManager):
    ...
    def apply_capacity_sf(self):
        ...
        nominal_receiver_heat_losses = (
            0.06  # 6% losses nominal heat losses, from gafurov2013
        )
        ...
        I_DNI_nom_ERA5 = gk.raster.interpolateValues(
            rk_weather.Era5Source.DNI_90_PERC_QUANT, self.locs, mode="near",
        )
        I_DNI_nom = (
            I_DNI_nom_ERA5
    ...
    def applyHTFHeatLossModel(
        ...
        if calculationmethod == "zero":
            ...
            _temperature[0, :] = (
                self.sim_data["surface_air_temperature"][0, :] + 100
            )  # initial temperature
            # losses
    ...
    def optimize_plant_size(
        ...
        for size in sizing_tuples:
            ...
            LCOE_EURct_per_kWh_el_3D[
                :, i_sm, i_tes
            ] = LCOE_EUR_per_kWh_el  # thats the output of the loop
    ...
    def optimize_heat_output_4D(self):
        ...
        dailyHeatStorable_Wh_4D = np.tensordot(
            dailyHeatStorable_Wh_3D, np.ones(self.opt_data["dimensions"][3]), axes=0,
        )
        ...
        dailyHeatDirect_Wh_4D = np.tensordot(
            dailyHeatDirect_Wh_3D, np.ones(self.opt_data["dimensions"][3]), axes=0,
        )
        ...
        self.opt_data[
            "annualHeat_Wh_3D"
        ] = annualHeat_Wh_3D  # dimensions: [placements, SM, TES]
        self.opt_data[
            "dailyHeatOutput_Wh_4D"
        ] = dailyHeatOutput_Wh_4D  # dimensions: [time(days), placements, SM, TES]
    ...
    def calculate_electrical_output(self, onlynightuse=True, debug_vars=False):
        ...
        if onlynightuse:
            ...
            operationalhours_per_day = np.maximum(
                operationalhours_per_day, 3
        ...
        if onlynightuse:
            ...
            Heat_directly_per_day_Wh = np.minimum(
                ...
                power_plant_max_heat_Wh
                - Heat_from_storage_used_per_day_Wh,  # maximum heat capable for the plant (cf_day=1)
            )
    ...
    def _get_totex(
        self, CAPEX_total_EUR_per_a, OPEX_EUR_per_a,
    ):
    ...
    def _get_totex_from_self(
        ...
        TOTEX_EUR_per_a = self._get_totex(
            CAPEX_total_EUR_per_a=CAPEX_total_EUR_per_a, OPEX_EUR_per_a=OPEX_EUR_per_a,
        )
        return TOTEX_EUR_per_a
