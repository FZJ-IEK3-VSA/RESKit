...
class PTRWorkflowManager(SolarWorkflowManager):
    ...
    def determine_area(self):
        ...
        if 'area' in columns and not 'aperture_area_m2' in columns and not 'land_area_m2' in columns:
            ...
            self.placements['aperture_area_m2'] = self.placements['land_area_m2'] * \
                self.ptr_data['SF_density_total']
        ...
        # only land_area_m2 in placements
            ...
            self.placements['land_area_m2'] = self.placements['aperture_area_m2'] / \
                self.ptr_data['SF_density_total']
    ...
    def check_ERA5_input(self):
        """Check inputs"""
        assert (self.sim_data["direct_horizontal_irradiance"].mean(
            axis=0) < 1500).all()
    ...
    def direct_normal_irradiance_from_trigonometry(self):
        ...
        sel = np.logical_or(
            sel, self.sim_data['direct_normal_irradiance'] > 1600)
    ...
    def apply_azimuth(self):
        ...
        if orientation == 'northsouth':
            ...
            self.placements['azimuth'] = self.placements['lat'].apply(
                apply_song2013)
    ...
    def apply_capacity_sf(self):
        ...
        # 6% losses nominal heat losses, from gafurov2013
        nominal_receiver_heat_losses = 0.06
        ...
        if len(self.placements) > 1:
            assert ((self.sim_data['HeattoHTF_W'].mean(
                axis=0) / self.placements.capacity_sf_W_th) < 1).all()
        else:
            assert (self.sim_data['HeattoHTF_W'].mean(
                axis=0) / self.placements.capacity_sf_W_th) < 1
    ...
    def calculateShadowLosses(self, SF_density: float = 0.383, method: str = 'wagner2011'):
        ...
        if method == 'wagner2011':
            ...
            self.sim_data['eta_shdw'] = np.minimum(np.abs(
                np.cos(np.deg2rad(self.sim_data['tracking_angle']))) / SF_density, 1)  # TODO
            ...
            self.sim_data['eta_shdw'] = np.sin(np.deg2rad(
                self.sim_data['solar_altitude_angle_degree'])) / (SF_density * np.cos(np.deg2rad(self.sim_data['theta'])))
    ...
    def calculateWindspeedLosses(self, max_windspeed_threshold: float = 14):
        ...
        self.sim_data['eta_wind'] = np.less_equal(
            self.sim_data['surface_wind_speed'], max_windspeed_threshold).astype(int)
    ...
    def applyHTFHeatLossModel(self, calculationmethod: str = 'gafurov2013', params: dict = {}):
        ...
        if calculationmethod == "zero":
            ...
            _losses = np.ones_like(self.sim_data['HeattoHTF_W'], dtype=float) * \
                params['relHeatLosses'] * params['ratedFieldOutputHeat_W']
            ...
            #K = cos(theta) * IAM
            _K = np.cos(np.deg2rad(
                self.sim_data['theta'])) * self.sim_data['IAM']
            ...
            if params['discretizationmethod'] == 'euler explicit':
                def simulation(
                    ...
                    maxHTFmeanTemperature = (
                        maxHTFTemperature + inletHTFTemperature)/2
                    ...
                    for i in range(0, temperature.shape[0]-1):
                        ...
                        losses[i, :] = losses[i, :] + \
                            add_losses_coefficient * deltaT * A
                        ...
                        is_first_max_heat = np.logical_and(
                            temperature[i-1, :] != maxHTFmeanTemperature, temperature[i, :] == maxHTFmeanTemperature)
                        ...
                        HeattoPlant[i, :] = np.maximum(
                            HeattoPlant[i, :] - is_first_max_heat * heat_flux_htf_heatup_last_timestep, 0)
                        ...
                        P_heating[i, :] = np.maximum(
                            (losses[i, :] - HeattoHTF[i, :]) * (temperature[i, :] == minHTFmeanTemperature), 0)
                ...
                def simulation(
                    ...
                    for i in range(0, temperature.shape[0]-1):
                        ...
                        for i_loc in range(0, HeattoHTF.shape[1]):
                            ...
                            p1 = (relTMplant * A) / deltat + K[i+1, i_loc] * b[0] * A * \
                                DNI[i+1, i_loc] + A * b[1] + \
                            ...
                            solutions_T = solutions_T_star - \
                                ambient_temperature[i+1, i_loc]
                            ...
                            # set temperature
                            temperature[i+1,
                                        i_loc] = solutions_T_filtered[0].real
                        ...
                        # calculate delta T
                        deltaT = temperature[i+1, :] - \
                            ambient_temperature[i+1, :]

                        # loss formula from greenius
                        losses[i+1, :] = K[i, :] * b[0] * deltaT * A * DNI[i, :] + A * (
                            b[1] * deltaT**1 + b[2] * deltaT**2 + b[3] * deltaT**3 + b[4] * deltaT**4)
                        # additional loss factor:
                        losses[i+1, :] = losses[i+1, :] + \
                            add_losses_coefficient * deltaT * A
                        #losses[i, :] = losses[i, :] * heatlossfactor + heatlossconstant
                        ...
                        # Heat to plant equals Heat input minus losses, when the plant is in operation mode (temperature = max temperature)
                        HeattoPlant[i, :] = np.maximum(
                            (HeattoHTF[i, :] - losses[i, :]) * (temperature[i, :] == maxHTFmeanTemperature), 0)
                        # because of discretization uncertainties, HeattoPlant can get negativ. So this will be prevented here. Looks odd, but is true
            ...
            if len(self.placements) > 1:
                assert ((self.sim_data['HeattoPlant_W'].mean(
                    axis=0) / self.sim_data['HeattoHTF_W'].mean(axis=0)) < 1).all()
            else:
                assert (self.sim_data['HeattoPlant_W'].mean(
                    axis=0) / self.sim_data['HeattoHTF_W'].mean(axis=0)) < 1
    ...
    def calculateParasitics(self, calculationmethod: str = 'gafurov2013', params: dict = {}):
        ...
        if calculationmethod == 'gafurov2013':
            ...
            # PL_sf_track
            PL_sf_track = params['PL_sf_track'] * P_pb_des * \
                (self.sim_data['solar_zenith_degree'] < 90)

            # PL_sf_pumping
            PL_sf_pumping = params['PL_sf_pumping'] * Q_sf_des * \
                np.power(self.sim_data['HeattoPlant_W'] / Q_sf_des, 3)
            # PL_plant_pumping
            PL_plant_pumping = params['PL_plant_pumping'] * \
            ...
            self.sim_data['Parasitics_solarfield_W_el'] = PL_sf_track + \
                PL_sf_pumping
            ...
            PL_sf_track = params['PL_sf_fixed_W_per_m^2_ap'] * \
                self.placements['aperture_area_m2'].values * \
                (self.sim_data['HeattoHTF_W'] > 0)
            PL_sf_pumping = params['PL_sf_pumping_W_per_m^2_ap'] * self.placements['aperture_area_m2'].values * \
                np.power(self.sim_data['HeattoPlant_W'] / Q_sf_des,
                         2)  # * 830/self.placements['I_DNI_nom_W_per_m2'].values), 2) #used for valiadaton, as 830 as DNI_des is used in reference data
        ...
        self.sim_data['Parasitics_W_el'] = self.sim_data['Parasitics_solarfield_W_el'] + \
            self.sim_data['Parasitics_plant_W_el']
        self.placements['Parasitics_solarfield_Wh_el_per_a'] = self.sim_data['Parasitics_solarfield_W_el'].sum(
            axis=0)
        self.placements['Parasitics_plant_Wh_el_per_a'] = self.sim_data['Parasitics_plant_W_el'].sum(
            axis=0)
    ...
    def calculateCapacityFactors(self):
        ...
        self.sim_data['capacity_factor_sf'] = self.sim_data['HeattoPlant_W'] / np.tile(
            self.placements['capacity_sf_W_th'], (self.sim_data['HeattoPlant_W'].shape[0], 1))
        self.sim_data['capacity_factor_heat_FP_sf'] = -1 * self.sim_data['P_heating_W'] / \
            np.tile(self.placements['capacity_sf_W_th'],
    ...
    def calculateEconomics_SolarField(self, WACC: float = 8, lifetime: float = 25,  calculationmethod: str = 'franzmann2021', params: dict = {}):
        ...
        self.sim_data['annuity'] = (
            WACC * (1 + WACC)**lifetime) / ((1+WACC)**lifetime - 1)

        dt = (self._time_index_[1] -
              self._time_index_[0]) / pd.Timedelta(hours=1)
        # calculate the average annual heat production
        self.placements['annualHeatfromSF_Wh'] = self.sim_data['HeattoPlant_W'].mean(
            axis=0) * ((self._time_index_[-1] - self._time_index_[0]) / pd.Timedelta(hours=1))
        ...
        Capex_SF_EUR_per_a = self.placements['CAPEX_SF_EUR'] * \
            self.sim_data['annuity']
        opexFix_SF_EUR_per_a = self.placements['CAPEX_SF_EUR'] * \
            params['OPEX_perc_CAPEX'] / 100

        # calculate opex
        dt = (self._time_index_[1] -
              self._time_index_[0]) / pd.Timedelta(hours=1)
        opexVar_SF_EUR_per_a = self.sim_data['Parasitics_solarfield_W_el'].sum(
            axis=0) / 1000 * dt * params['electricity_price_EUR_per_kWh']

        # calculate annual Totex
        self.placements['Totex_SF_EUR_per_a'] = Capex_SF_EUR_per_a + \
            opexFix_SF_EUR_per_a + opexVar_SF_EUR_per_a

        # Cost relative to Heat
        self.placements['LCO_Heat_SF_EURct_per_kWh'] = self.placements['Totex_SF_EUR_per_a'] / \
            self.placements['annualHeatfromSF_Wh'] * \
            1E2 * 1E3  # EUR/Wh to EURct/kWh
    ...
    def optimize_plant_size(self, onlynightuse=True, fullvariation=False, debug_vars=False):
        ...
        if debug_vars:
            dailyHeatOutput_Wh_4D = np.nan*np.ones(shape=(len(np.unique(self.time_index.date)), len(
                self.placements), len(self.sm), len(self.tes)))  # TODO: remove
            TOTEX_EUR_per_a_3D = np.nan * \
                np.ones(shape=(len(self.placements), len(
                    self.sm), len(self.tes)))  # TODO: remove
            Power_output_plant_net_Wh_per_a_3D = np.nan * \
                np.ones(shape=(len(self.placements), len(
                    self.sm), len(self.tes)))  # TODO: remove
        LCOE_EURct_per_kWh_el_3D = np.nan * \
            np.ones(shape=(len(self.placements), len(self.sm), len(self.tes)))
        for size in sizing_tuples:
            ...
            # aggregate to daily
            aggregate_by_day = np.eye(
                len(np.unique(self.time_index.date))).repeat(24, axis=1)
            ...
            Heat_heating_sf_daily_Wh_th = np.einsum(
                'ij,jk', aggregate_by_day, self.sim_data['P_heating_W'])  # 13
            ...
            Heat_unstored_daily_Wh_th = np.maximum(
                Heat_stored_daily_Wh_th * self.ptr_data['storage_efficiency_1'] - Heat_heating_sf_daily_Wh_th, 0)  # 13
            P_backup_heating_daily_Wh_el = np.maximum(
                Heat_heating_sf_daily_Wh_th - Heat_stored_daily_Wh_th * self.ptr_data['storage_efficiency_1'], 0)  # 13
            ...
            Heat_max_des_powerplant_daily_Wh_th = Heatflux_powerplant_des_input_W_th.values * \
                operationalhours_per_day  # h/day

            # calculate actually used heat
            Heat_total_used_daily_Wh_th = np.minimum(
                (Heat_direct_powerplant_daily_Wh_th + Heat_unstored_daily_Wh_th), Heat_max_des_powerplant_daily_Wh_th)
            del Heat_direct_powerplant_daily_Wh_th, Heat_unstored_daily_Wh_th, Heat_max_des_powerplant_daily_Wh_th

            # remember that one
            if debug_vars:
                dailyHeatOutput_Wh_4D[:, :, i_sm,
                                      i_tes] = Heat_total_used_daily_Wh_th
            ...
            efficiency_daily_1 = self._get_plant_efficiency(
                rel_load_plant=rel_load_plant_daily_1, eta_nom=self.ptr_data['eta_powerplant_1'])
            ...
            # plant parasitics
            self.sim_data_daily['Parasitics_plant_daily_Wh_el'] = Parasitics_plant_daily_Wh_el = np.einsum(
                'ij,jk', aggregate_by_day, self.sim_data['Parasitics_W_el']) * 1  # h #13
            ...
            if debug_vars:
                Power_output_plant_net_Wh_per_a_3D[:, i_sm,
                                                   i_tes] = Power_output_plant_net_Wh_per_a
            ...
            TOTEX_EUR_per_a = self._get_totex_from_self(
                sm_manipulation=sm, tes_manipulation=tes, P_aux_manipulation=P_backup_heating_daily_Wh_el.sum(axis=0))
            ...
            LCOE_EUR_per_kWh_el = np.nan_to_num(
                TOTEX_EUR_per_a.values / Power_output_plant_net_Wh_per_a * 1E5, nan=0)  # EUR/Wh --> EURct/kWh

            # thats the output of the loop
        ...
        self.placements['storage_capacity_kWh_th'] = self.placements['capacity_sf_W_th'] / \
            sm_opt * tes_opt / 1000
        self.placements['power_plant_capacity_W_el'] = self.placements['capacity_sf_W_th'] / \
            sm_opt * self.ptr_data['eta_powerplant_1']
    ...
    def optimize_heat_output_4D(self):
        ...
        #dimensions: [time(days), placements, SM, TES]
        dailyHeatStored_Wh_4D = np.minimum(
            dailyHeatStorable_Wh_4D, storage_Size_Wh_4D)
        ...
        dailyHeatOutput_Wh_4D = np.minimum(
            (dailyHeatDirect_Wh_4D + dailyHeatStored_Wh_4D * self.ptr_data['storage_efficiency_1']), maxDailyHeatPlant_W_4D)
    ...
    def calculateEconomics_Plant_Storage_4D(self):
        ...
        #dimensions: [SM, TES]
        CAPEX_EUR_per_a_kW_SF_2D = CAPEX_EUR_per_kW_SF_2D * \
            self.sim_data['annuity']
        varOPEX_EUR_per_a_kW_SF_2D = CAPEX_EUR_per_kW_SF_2D * \
            self.ptr_data['OPEX_perc_CAPEX'] / 100
        fixOPEX_EUR_per_a_kW_SF_2D = 0
        del CAPEX_EUR_per_kW_SF_2D

        #dimensions: [SM, TES]
        TOTEX_Plant_storage_EUR_per_a_kw_SF_2D = CAPEX_EUR_per_a_kW_SF_2D + \
            varOPEX_EUR_per_a_kW_SF_2D + fixOPEX_EUR_per_a_kW_SF_2D
    ...
    def optimal_Plant_Configuration_4D(self):
        ...
        rel_load_plant_4D = 0.5 + 0.5 * \
            (self.opt_data['dailyHeatOutput_Wh_4D'] /
             Q_plant_des_Wh_per_day_4D)

        #dimensions: [time(days), placements, SM, TES]
        efficiency_daily_averaged_1_4D = self._get_plant_efficiency(
            rel_load_plant=rel_load_plant_4D, eta_nom=self.ptr_data['eta_powerplant_1'])
        del rel_load_plant_4D

        efficiency_daily_averaged_1_4D = self.ptr_data['eta_powerplant_1']
        #dimensions: [time(days), placements, SM, TES]
        self.opt_data['dailyPowerOutput_Wh_4D'] = efficiency_daily_averaged_1_4D * \
            self.opt_data['dailyHeatOutput_Wh_4D']
        del efficiency_daily_averaged_1_4D
        self.opt_data['annualPowerOutput_Wh_3D'] = self.opt_data['dailyPowerOutput_Wh_4D'].sum(
            axis=0)

        LCOE_EUR_per_Wh = self.opt_data['TOTEX_CSP_EUR_per_a_3D'] / \
    ...
    def calculate_electrical_output(self, onlynightuse=True, debug_vars=False):
        ...
        if hasattr(self, 'aggregate_by_day'):
            ...
            aggregate_by_day = np.eye(365).repeat(24, axis=1)
        HeattoPlant_per_day_Wh = np.einsum(
            'ij,jk', aggregate_by_day, self.sim_data['HeattoPlant_W']) * dt
        Parasitics_plant_per_day_Wh_el = np.einsum(
            'ij,jk', aggregate_by_day, self.sim_data['Parasitics_W_el']) * dt  # issue #13
        Heat_heating_sf_daily_Wh_th = np.einsum(
            'ij,jk', aggregate_by_day, self.sim_data['P_heating_W'])  # issue #13
        ...
        power_plant_max_heat_Wh = self.placements['power_plant_capacity_W_el'].values / \
            self.ptr_data['eta_powerplant_1'] * operationalhours_per_day
        ...
        if debug_vars:
            self.placements['avrg_sf_efficiency_1'] = self.sim_data['HeattoPlant_W'].sum(
                axis=0) / (self.placements['aperture_area_m2'] * self.sim_data['direct_normal_irradiance'].sum(axis=0))
            self.placements['Heat_after_curtailment_1'] = Heat_total_per_day_Wh.sum(
                axis=0) / self.sim_data['HeattoPlant_W'].sum(axis=0)

            self.placements['heat_dni_Wh'] = self.placements['aperture_area_m2'] * \
                self.sim_data['direct_normal_irradiance'].sum(axis=0)
            self.placements['heat_losses_sf_Wh'] = (
                self.placements['aperture_area_m2'] * self.sim_data['direct_normal_irradiance'].sum(axis=0)) - self.sim_data['HeattoPlant_W'].sum(axis=0)
            self.placements['heat_losses_curtailment_storage_Wh'] = (
                self.sim_data['HeattoPlant_W']).sum(axis=0) - (Heat_stored_per_day_Wh).sum(axis=0)
            self.placements['heating_sf_from_storage'] = Heat_from_storage_per_day_Wh.sum(
                axis=0) - Heat_from_storage_net_per_day_Wh.sum(axis=0)
            self.placements['heating_sf_from_elec'] = self.sim_data_daily['P_backup_heating_daily_Wh_el'].sum(
                axis=0)
            self.placements['heat_losses_curtailment_plant_Wh'] = (
                Heat_from_storage_net_per_day_Wh - Heat_from_storage_used_per_day_Wh).sum(axis=0)
        ...
        share = np.nan_to_num(Heat_directly_per_day_Wh /
                              Heat_total_per_day_Wh, 0)
        Power_net_bound_per_day_Wh = np.maximum(
            Power_gross_bound_per_day_Wh - (Parasitics_plant_per_day_Wh_el * share), 0)

        # dispatchable
        Power_gross_dispatchable_per_day_Wh = Heat_from_storage_used_per_day_Wh * \
            efficiency_daily_averaged_1
        # with np.seterr(divide='ignore', invalid='ignore'):
        share = np.nan_to_num(
            Heat_from_storage_used_per_day_Wh/Heat_total_per_day_Wh, 0)
        Power_net_dispatchable_per_day_Wh = np.maximum(
            Power_gross_dispatchable_per_day_Wh - (Parasitics_plant_per_day_Wh_el * share), 0)

        # add up for total output
        Power_net_total_per_day_Wh = Power_net_bound_per_day_Wh + \
            Power_net_dispatchable_per_day_Wh

        if debug_vars:
            self.placements['mean_gross_turbine_efficiency_1'] = (Power_gross_dispatchable_per_day_Wh.sum(
                axis=0) + Power_gross_bound_per_day_Wh.sum(axis=0)) / Heat_total_per_day_Wh.sum(axis=0)
            self.placements['turbine_gross_to_net'] = Power_net_total_per_day_Wh.sum(
                axis=0) / (Power_gross_dispatchable_per_day_Wh.sum(axis=0) + Power_gross_bound_per_day_Wh.sum(axis=0))

            self.placements['heat_losses_turbine_plant_Wh'] = Heat_total_per_day_Wh.sum(
                axis=0) - (Power_gross_dispatchable_per_day_Wh.sum(axis=0) + Power_gross_bound_per_day_Wh.sum(axis=0))
            self.placements['heat_losses_turbine_aux_Wh'] = (Power_gross_dispatchable_per_day_Wh.sum(
                axis=0) + Power_gross_bound_per_day_Wh.sum(axis=0)) - Power_net_total_per_day_Wh.sum(axis=0)

        # get avrg cf
        Power_net_total_Wh_per_a = Power_net_total_per_day_Wh.sum(axis=0)
        steps_per_year = pd.Timedelta(
            hours=8760) / (self._time_index_[1] - self._time_index_[0])
        ...
        self.placements['Power_net_bound_perc_per_a'] = np.nan_to_num(
            Power_net_bound_per_day_Wh.sum(axis=0) / Power_net_total_Wh_per_a) * 100  # %
    ...
    def calculate_LCOE(self):
        ...
        self.placements['lcoe_EURct_per_kWh_el'] = TOTEX_EUR_per_a / \
            self.placements['Power_net_total_Wh_per_a'] * \
            1E2 * 1E3  # EUR/WH to EURct/kWh
    ...
    def calculateSolarPositionfaster(self):
        ...
        _time = pd.DatetimeIndex(
            np.tile(self.time_index.values, self._numlocations))
    ...
    def _applyVariation(self):
        ...
        if dni_factor_name in self.placements.columns and t_amb_offset_name in self.placements.columns:
            ...
            # do manipulation
            f_DNI_np = np.tile(
                self.placements[dni_factor_name].values.T, (8760, 1))
            self.sim_data['direct_normal_irradiance'] = self.sim_data['direct_normal_irradiance'] * f_DNI_np

            d_T_np = np.tile(
                self.placements[t_amb_offset_name].values.T, (8760, 1))
            self.sim_data['surface_air_temperature'] = self.sim_data['surface_air_temperature'] + d_T_np

            # get annual values
            self.placements['mean_DNI_W_per_m2'] = self.sim_data['direct_normal_irradiance'].mean(
                axis=0)
            self.placements['mean_T_amb_K'] = self.sim_data['surface_air_temperature'].mean(
                axis=0)
        else:
    ...
    def _get_capex(
        ...
        CAPEX_Plant_EUR = c_plant_EUR_per_kW_el * \
            Power_powerplant_des_W_el / 1000  # W --> kW

        # capex storage
        CAPEX_Storage_EUR = c_storage_EUR_per_kWh_th * \
            Heat_storage_des_kWh_th / 1000  # Wh-->kWh
        ...
        # indirect costs
        CAPEX_Indirect_EUR = (CAPEX_Plant_EUR + CAPEX_Storage_EUR + CAPEX_Field_EUR +
                              CAPEX_Land_EUR) * c_indirect_cost_perc_per_direct_Capex/100

        CAPEX_total_EUR = (CAPEX_Plant_EUR + CAPEX_Storage_EUR +
                           CAPEX_Field_EUR + CAPEX_Land_EUR + CAPEX_Indirect_EUR)
    ...
    def _get_opex(
        ...
        OPEX_var_EUR_per_a = electricity_price_EUR_per_kWh * \
            auxilary_power_Wh_per_a / 1000 * 1  # W --> kWh
    ...
    def _get_totex_from_self(self, sm_manipulation=None, tes_manipulation=None, P_aux_manipulation=None):
        ...
        if sm_manipulation is None:
            ...
            assert isinstance(sm_manipulation, int) or isinstance(sm_manipulation, float) or isinstance(
                tes_manipulation, np.int64) or isinstance(tes_manipulation, np.int32)
            sm = sm_manipulation
        ...
        if tes_manipulation is None:
            ...
            assert isinstance(tes_manipulation, np.int32) or isinstance(tes_manipulation, float) or isinstance(
                tes_manipulation, int) or isinstance(tes_manipulation, np.int64)
        ...
        if P_aux_manipulation is None:
            ...
            P_aux = self.sim_data_daily['P_backup_heating_daily_Wh_el'].sum(
                axis=0)
        else:
            ...
            assert len(P_aux_manipulation) == len(
                self.placements)  # same len as placements
