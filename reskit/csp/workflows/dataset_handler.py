...
class dataset_handler():
    ...
    def _get_opt_HTF_matrix(self):
        ...
        if os.path.isfile(path):
            ...
            htf_opt_matrix.index = htf_opt_matrix.index.astype(
                float).astype(int)
            htf_opt_matrix.columns = htf_opt_matrix.columns.astype(
                float).astype(int)
        else:
    ...
    def _calc_opt_HTF_matrix(self) -> pd.DataFrame:
        ...
        for datasetname in datasetnames:
            ...
            out = out.placements[['mean_T_amb_K',
                                  'mean_DNI_W_per_m2', variable_name]]
        ...
        for key_comp in pivots:
            ...
            for key_ref in pivots:
                ...
                isnotmin = pivots[key_ref] <= pivots[key_comp]
        ...
        if hel_exists and the_exists:
            if hel_exists and sol_exists:
                min_Heliosol = min_Heliosol & (
                    pivot_Heliosol < pivot_Therminol)
            else:
        ...
        if hel_exists and the_exists:
            if hel_exists and sol_exists:
                min_SolarSalt = min_SolarSalt & (
                    pivot_SolarSalt < pivot_Therminol)
            else:
        ...
        if the_exists and hel_exists:
            if the_exists and sol_exists:
                min_Therminol = min_Therminol & (
                    pivot_Therminol < pivot_Heliosol)
            else:
