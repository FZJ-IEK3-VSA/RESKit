        ...
        for dataset in datasets:
            ...
            if not 'datasetname' in output_dataset.variables:
                output_dataset['datasetname'] = (
                    output_dataset['lon']*0).astype(str)
...
def CSP_PTR_ERA5_specific_dataset(
    ...
    if verbose:
        tic_start = time.time()
        print('Simulation started for {n_placements} placements.'.format(
            n_placements=len(wf.placements)), flush=True)
    ...
    if verbose:
        tic_read = time.time()
        print('Data read in within {dt}s.'.format(
            dt=str(tic_read-tic_start)), flush=True)
    ...
    if not debug_vars:
        del wf.sim_data['surface_wind_speed'], wf.sim_data['tracking_angle'], wf.sim_data[
            'solar_altitude_angle_degree'], wf.sim_data['direct_horizontal_irradiance']
        del wf.sim_data['eta_wind'], wf.sim_data['eta_degradation'], wf.sim_data['eta_shdw']

    if verbose:
        tic_pre = time.time()
        print('Preanalysis within {dt}s.'.format(
            dt=str(tic_pre-tic_read)), flush=True)
    ...
    if verbose:
        ...
        print('Solar field simulation done in {dt}s.'.format(
            dt=str(tic_sf_sim-tic_pre)), flush=True)
        print('Starting optimizing plant electric output.', flush=True)

    wf.optimize_plant_size(onlynightuse=onlynightuse,
    ...
    if verbose:
        ...
        print('Optimal Sizing done in  {dt}s.'.format(
            dt=str(tic_opt_plant-tic_sf_sim)), flush=True)
    ...
    if verbose:
        tic_final = time.time()
        print('Total simulation done in {dt}s.'.format(
            dt=str(tic_final-tic_start)), flush=True)
