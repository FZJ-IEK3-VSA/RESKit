...
def main(datasetname, validation_output_folder):
    ...
    for i in range(len(placements)):
        ...
        print('Calculating Placement {i} of {max}: {name}'.format(i=i+1, max=str(placements.shape[0]), name=str(placement['Name'])))
        #get tile numbers
    ...
    out_reskit.drop("tile x / y").to_netcdf(os.path.join(validation_output_folder, f"sim_results_reskit_{htf_greenius}.nc4"))
    # %% Load Greenius Data
    ...
    for i, path_greenius in enumerate(paths_greenius):
        ...
        df_data_greenius['Q losses'] = df_data_greenius['Q Heat'] + df_data_greenius['Q Vessel'] + df_data_greenius['Q Pipe']
    ...
    metrics = [RMSE, absError, relabsError, meanDev, relmeanDev, stdDifference , corrCoefficient, Nash_Sutcliffe]#, DCCA]
    ...
    full_metric_report_summed.to_csv(os.path.join(validation_output_folder, f"full_metric_report_summed_{htf_greenius}.csv"))
...
def DCCA(ser1, ser2):
    try:
        ...
        #compute rho index
        wins = np.array([24], dtype = 'int64') #only take a window size of 1 day (24h) , 7*24, 30*24, 91*24])
