...
def main(datasetname, validation_output_folder):
    ...
    path_locations = os.path.join(os.path.dirname(
        __file__), "Validation_Locations.xlsx")
    ...
    for i in range(len(placements)):
        ...
        print('Calculating Placement {i} of {max}: {name}'.format(
    ...
    out_reskit.drop("tile x / y").to_netcdf(os.path.join(validation_output_folder,
                                                         f"sim_results_reskit_{htf_greenius}.nc4"))
    # %% Load Greenius Data

    path_greenius_glob = os.path.join(
        os.path.dirname(__file__),
        "greenuis_reference_data",
        f"*{htf_greenius}*.xlsx"
    )
    ...
    for i, path_greenius in enumerate(paths_greenius):
        ...
        df_data_greenius = pd.read_excel(
            path_greenius,
            skiprows=range(1, 4),
        )
        df_data_greenius['Q losses'] = df_data_greenius['Q Heat'] + \
            df_data_greenius['Q Vessel'] + df_data_greenius['Q Pipe']
    ...
    vars_greenius = 'Q abs|T HTFmean|Q out|Q losses|QFP Aux|W parField'.split(
        '|')
    metrics = [RMSE, absError, relabsError, meanDev, relmeanDev,
               stdDifference, corrCoefficient, Nash_Sutcliffe]  # , DCCA]
    ...
    for i_placement in range(len(placements)):
        ...
        for var_reskit, var_greenius, f_greenius in zip(vars_reskit, vars_greenius, fs_greenius):
            data_reskit = out_reskit.sel(location=i_placement)[
                var_reskit].values
            ...
            df_temp = pd.DataFrame(
                [values], columns=columns, index=[i_placement])
    ...
    full_metric_report.to_csv(os.path.join(
        validation_output_folder, f"full_metric_report_{htf_greenius}.csv"))
    full_metric_report_summed.to_csv(os.path.join(
    ...
    plot_correlation(
        ...
        savepath=os.path.join(validation_output_folder,
                              f"corr_Q_out_{htf_greenius}.png")
    )

    plot_correlation(
        x=greenius_ref_dict[i_placement]["T HTFmean"].values,
        y=out_reskit.sel(location=i_placement)[
            "HTF_mean_temperature_C"].values,
        x_label="HTF_mean_temperature Greenius [°C]",
        y_label="HTF_mean_temperature RESKit [°C]",
        title="Correlation T out",
        savepath=os.path.join(validation_output_folder,
                              f"corr_T_out_{htf_greenius}.png")
    )
    ...
    out_reskit['P_par_field_W'] = out_reskit['Parasitics_solarfield_W_el'] - \
        out_reskit['P_heating_W']
    plot_time_series(
        i_placement=0,
        greenius_ref_dict=greenius_ref_dict,
        out_reskit=out_reskit,
...
def plot_time_series(i_placement, greenius_ref_dict, out_reskit, varnames1, varnames2, factors, start, stop, placements, htf_greenius):
    location = placements.loc[i_placement].Name

    x = range(start, stop)

    data_gr_heat = greenius_ref_dict[i_placement][varnames1[0]
                                                  ].values[start:stop]
    data_re_heat = out_reskit.sel(location=i_placement)[
        varnames1[1]].values[start:stop]*factors[0]

    data_gr_temp = greenius_ref_dict[i_placement][varnames2[0]
                                                  ].values[start:stop]
    data_re_temp = out_reskit.sel(location=i_placement)[
        varnames2[1]].values[start:stop]*factors[1]
    ...
    savepath = os.path.join(validation_output_folder,
                            f"ts_plot_{htf_greenius}_{y1_axis}_{y2_axis}.png")
...
if __name__ == "__main__":
    main(
        ...
        validation_output_folder=validation_output_folder,
    )
