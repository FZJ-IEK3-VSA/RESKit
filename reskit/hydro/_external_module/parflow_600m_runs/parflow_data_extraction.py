...
def find_depth_index(depth, soil_layer_bnds):
    ...
    if depth > max(depth_list):
        print(
            "the specified depth value is out of the range of the dataset, min depth, max depth: ",
            min(depth_list),
            max(depth_list),
        )
        print("exit now")
    ...
    for i in range(len(depth_list) - 1):
        ...
        if depth_list[i] >= depth > depth_list[i + 1]:
            ...
            print(
                "your specified depth lies within a depth-layer/on a boundary: showing boundaries and returning index for the layer"
            )
...
def data_extraction(runctrl_file, output_format):
    ...
    with open(runctrl_file, "r") as f:
        ...
        for ilocation in locations:
            ...
            print(
                f"location ID, lon, lat, depth, sim-data: {locationID}, {locationLon}, {locationLat}, {locationDepth}, {locationData}"
            )
            ...
            # dim0=y, dim1=x
            dist = spher_dist(
                np.deg2rad(simLons), np.deg2rad(simLats), np.deg2rad(locationLon), np.deg2rad(locationLat)
            )
            mapped_idx = np.unravel_index(np.argmin(dist, axis=None), dist.shape)
            print(
                f"POI: index x-y and lon-lat on model grid: {mapped_idx[1]}, {mapped_idx[0]}, {simLons[mapped_idx[0], mapped_idx[1]]}, {simLats[mapped_idx[0], mapped_idx[1]]}"
            )
            ...
            if indicator[mapped_idx[0], mapped_idx[1]] in [19, 20, 21]:
                ...
                closest_indices = list(
                    zip(
                        sorted_indices_2d[0][1:nr_of_neighbour_grid_pts],
                        sorted_indices_2d[1][1:nr_of_neighbour_grid_pts],
                    )
                )
                ...
                for j in range(len(closest_indices)):
                    if indicator[sorted_indices_2d[0][j], sorted_indices_2d[1][j]] not in [19, 20, 21]:
                        ...
                        print(
                            "alternative, recommended coordinate pairs close-by, not on river, lake, or ocean, Lon-Lat [dec deg], 5 digits, edit your JASON file accordingly: ",
                            lon_f,
                            lat_f,
                        )
                        k += 1
                if k == 0:
                    print(
                        "there are no alterntive grid elements around your original POI which are NOT on a river, lake, or ocean grid element"
                    )
            ...
            with Dataset(locationData, "r") as ncData:
                ...
                if output_format == "csv":
                    ...
                    writer.writerow(
                        [
                            "data server: ",
                            "https://datapub.fz-juelich.de/slts/FZJ_ParFlow_DE06_hydrologic_forecasts/index.html",
                        ]
                    )
                    ...
                    if ncData.variables[variable].ndim == 3:
                        ...
                        # 3D variable with value representative for a depth layer
                            ...
                            writer.writerow(
                                ["depth (mid-point of soil layer):", f"{(lower_boundary + upper_boundary) / 2.0} m"]
                            )
