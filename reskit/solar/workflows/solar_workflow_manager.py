...
class SolarWorkflowManager(WorkflowManager):
    ...
    def preprocess_bifaciality_factor(
        ...
        if self.module is not None and "Bifacial" in self.module.index:
            ...
            if ((bifaciality_factor > 0) != module_bifacials).any():
                print(
                    f"NOTE: Module bifacial information and bifaciality according to bifaciality factors do not align for at least one plant",
                    flush=True,
                )
        ...
        if self.module is not None and "bifaciality_factor" in self.module.index:
            ...
            if not np.isclose(bifaciality_factor, self.module["bifaciality_factor"]).all():
                print(
                    f"NOTE: Module has bifaciality_factor information but it does not match the provided bifaciality_factor argument for all locations.",
                    flush=True,
                )
    ...
    def preprocess_ground_coverage_ratio(
        ...
        obj
            reference to the invoking SolarWorkflowManager object
        """
        assert fallback is None or isinstance(fallback, float) and 0 < fallback <= 1, (
            "gcr 'fallback' must be a float >0 and <=1."
        )
        ...
        for conv in np.unique(conventions[~pd.isna(conventions)]):
            ...
            if isfile(conv):
                ...
                try:
                    ...
                    raise RuntimeError(
                        f"convention is an existing file but data could not be extracted via gk.raster.interpolateValues(): {e}"
                    )
            elif conv == "winter_solstice_rule":
                assert self.tracking == "fixed", (
                    f"'winter_solstice_rule' convention can only be applied to 'fixed', here '{self.tracking}' tracking."
                )
                iter_row_pitches, iter_gcrs = (
                    rk_solar_core.system_design.location_to_gcr_and_row_pitch_winter_solstice_rule(
                ...
                raise NotImplementedError(
                    f"Location-to-GCR convention '{conv}' is not implemented yet. Choose from: 'winter_solstice_rule', 'tonita_et_al_2023' or a raster filepath with GCR information."
                )
            ...
            if np.isnan(iter_gcrs).any():
                ...
                if fallback is None:
                    ...
                    iter_gcrs[np.isnan(iter_gcrs)] = (np.ones(shape=iter_gcrs.shape) * fallback)[np.isnan(iter_gcrs)]
        ...
        if fallbacks_applied > 0:
            warnings.warn(
                f"GCR for {fallbacks_applied} out of {self.locs.count} placements could not be extracted from desired convention and has been replaced with fallback value {fallback}."
            )
    ...
    def preprocess_elevation(
        ...
        obj
            reference to the invoking SolarWorkflowManager object
        """
        assert fallback is None or isinstance(fallback, int), (
        ...
        for fp in np.unique(filepaths[~pd.isna(filepaths)]):
            if not isfile(fp):
                raise FileNotFoundError(
                    f"elevation contains string entries which must be existing DEM rasters but cannot find file: {fp}"
                )
            ...
            iter_ext = gk.Extent.fromLocationSet(
                gk.LocationSet(self.locs[mask])
            )  # LocationSet can only be in EPSG:4326 as per geokit implementation
            ...
            if clipped_elev is None:
                ...
                if np.isnan(iter_vals).any():
                    ...
                    iter_vals_near = gk.raster.interpolateValues(clipped_elev, self.locs[mask], mode="near")
            ...
            if np.isnan(iter_vals).any():
                ...
                if fallback is None:
                    ...
                    iter_vals[np.isnan(iter_vals)] = (np.ones(shape=iter_vals.shape) * fallback)[np.isnan(iter_vals)]
            # last set the extracted elevations from this iteration in the main elevation array
            elevation[mask] = iter_vals
        # warn of unavailable elevation data if required
        if fallbacks_applied > 0:
            warnings.warn(
                f"Elevation for {fallbacks_applied} out of {self.locs.count} placements could not be extracted from provided DEM files and has been replaced with fallback value {fallback}m."
            )
    ...
    def preprocess_fixed_module_tilt(
        ...
        try:
            ...
            if "must not have None values" in str(e):
                raise ValueError(f"{e} This is required when tracking='fixed'.") from e
            raise
        ...
        for conv in np.unique(conventions[~pd.isna(conventions)]):
            ...
            if np.isnan(iter_vals).any():
                ...
                raise ValueError(
                    f"module_tilt values could not be extracted via required '{conv}' convention for all locations."
                )
    ...
    def preprocess_fixed_module_azimuth(
        ...
        for conv in np.unique(conventions[~pd.isna(conventions)]):
            ...
            if np.isnan(iter_vals).any():
                # we have nans in the data that we just extracted
                raise ValueError(
                    f"module_azimuth values could not be extracted via required '{conv}' convention for all locations."
                )
    ...
    def preprocess_pvrow_height(
        ...
        if self.tracking == "singleaxis":
            if not "max_tracking_angle" in self.plant_parameters_processed:
                raise AttributeError(
                    f"'max_tracking_angle' is a required attributes of self.plant_parameters_processed when tracking = 'singleaxis', run preprocess_tracking_angle() first."
                )
            if not "backtracking" in self.plant_parameters_processed:
                raise AttributeError(
                    f"'backtracking' is a required attributes of self.plant_parameters_processed when tracking = 'singleaxis', run preprocess_backtracking() first."
                )
        elif self.tracking == "fixed" and not "module_tilt" in self.plant_parameters_processed:
            raise AttributeError(
                f"'module_tilt' is a required attributes of self.plant_parameters_processed when tracking = 'fixed', run preprocess_fixed_module_tilt() first."
            )
        if not "pvrow_width_sloped" in self.plant_parameters_processed:
            raise AttributeError(
                f"'pvrow_width_sloped' is a required attributes of self.plant_parameters_processed, run configure_cec_module() first."
            )
        ...
        angles_deg = (
            self.plant_parameters_processed["module_tilt"]
            if self.tracking == "fixed"
            else self.plant_parameters_processed["max_tracking_angle"]
        )
        center_to_lower_edge_vertical = (
            1 / 2 * (self.plant_parameters_processed["pvrow_width_sloped"] * np.sin(np.radians(angles_deg)))
        )
        ...
        if not np.all(pvrow_height - 1 / 2 * self.plant_parameters_processed["pvrow_width_sloped"] > 0):
            ...
            raise ValueError(
                f"Given pvrow_height values do not lead to a sufficient ground clearance for all locations under the given module size and all potential module angles (incl. vertical)."
            )
    ...
    def preprocess_ground_albedo(
        ...
        obj
            reference to the invoking SolarWorkflowManager object
        """
        assert isinstance(fallback, float) and 0 < fallback < 1, "fallback_albedo must be a float between 0.0 and 1.0"
        ...
        landcover_name = None
        if (
            isinstance(ground_albedo, (tuple, list))
            and len(ground_albedo) == 2
            and all([isinstance(x, str) for x in ground_albedo])
        ):  # TODO can we somehow allow the filepath per location?
            ...
            if ground_albedo[0] in ground_cover_albedos["dataset_classtype_mapper"]:
                if isfile(ground_albedo[1]):
                    ...
                    raise ValueError(
                        f"Tuple or list with len = 2 was passed as ground_cover with known landcover name '{ground_albedo[0]}' but filepath as second entry does not exist: '{ground_albedo[1]}'"
                    )
            elif isfile(ground_albedo[1]):
                # the file exists but we have no known landcover name
                raise ValueError(
                    f"Tuple or list with len = 2 was passed as ground_cover with existing filepath as second entry but unknown landcover name: '{ground_albedo[0]}'"
                )
        # an Iterable of tuples has not been implemented yet
            ...
            raise NotImplementedError(
                "An iterable containing at least one tuple was passed as ground_albedo. Not implemented yet."
            )
        ...
        if any(isinstance(x, str) for x in filepaths) and landcover_name is None:
            raise TypeError(
                "ground_albedo scalar or array values may not be/contain strings, landcover dataset paths must be passed as single tuple/list: (landcover_name, landcover_path)"
            )
        ...
        def _get_ground_albedo_from_landcover(landcover_name: str, landcover_path: str, fallback_albedo: float):
            ...
            if landcover_name not in ground_cover_albedos["dataset_classtype_mapper"]:
                raise KeyError(
                    f"Unknown landcover_name '{landcover_name}'. Select from: {', '.join(ground_cover_albedos['dataset_classtype_mapper'].keys())}"
                )
            ...
            LCclasses = np.atleast_1d(
                gk.raster.interpolateValues(source=landcover_path, points=self.locs, pointSRS="latlon", mode="near")
            )
            ...
            if np.isnan(LCclasses).any():
                print(
                    f"Landcover raster has missing cell values, will be filled with fallback_albedo: {fallback_albedo}"
                )
        ...
        for fp in np.unique(filepaths[~pd.isna(filepaths)]):
            # get the azimuths only for the affected locations
            mask = filepaths == fp
            iter_vals = np.asarray(
                _get_ground_albedo_from_landcover(
        ...
        if np.any(consider_snow_albedo):
            ...
            if not all(
                [var in self.sim_data for var in ["snow_albedo", "snow_depth_water_equivalent", "snow_density"]]
            ):
                raise AttributeError(
                    "consider_snow_albedo=True but no 'snow_albedo', 'snow_depth_water_equivalent' and/or 'snow_density' data found in sim_data attribute."
                )
            # mask only the timesteps with actual snow (>= 1cm) on the ground
            snow_mask = (
                self.sim_data["snow_depth_water_equivalent"] * (1000 / self.sim_data["snow_density"]) >= 0.01
    ...
    def determine_solar_position(self, lon_rounding=1, lat_rounding=1, elev_rounding=-2):
        ...
        if "elevation" not in self.plant_parameters_processed:
            raise AttributeError(
                f"'elevation' attribute missing in self.plant_parameters_processed, run self.preprocess_elevation() first."
            )
    ...
    def filter_positive_solar_elevation(self):
        ...
        self._time_sel_ = (self.sim_data["apparent_solar_zenith"] < 95).any(
            axis=1
        )  # allow a few degrees below horizon to account for diffuse radiation after sunset and possibly slightly negative horizon angles
    ...
    def direct_normal_irradiance_from_trigonometry(self):
        ...
        if "apparent_solar_zenith" not in self.sim_data:
            raise AttributeError(
                "'apparent_solar_zenith' must be processed first e.g. via wfm.determine_solar_position()"
            )
    ...
    def preprocess_horizon_profile(
        ...
        **kwargs
        ...
        if "local_horizon_profile" not in self.plant_parameters_processed:
            ...
            self.calculate_local_horizon_based_on_hillslope(azimuthal_stepsize=azimuthal_stepsize, allow_negative=True)
    ...
    def preprocess_distant_horizon_profile(
        ...
        **kwargs
        ...
        if distant_horizon_profile.size == 1:  # scalar
            ...
            distant_horizon_profile = np.full(
                (self.locs.count, 1), distant_horizon_profile, dtype=object
            )  # always use 2D view
            ...
            if distant_horizon_profile.size == self.locs.count and np.all(is_none | is_str | is_zero):
                ...
                distant_horizon_profile = np.broadcast_to(
                    distant_horizon_profile[None, :], (self.locs.count, distant_horizon_profile.size)
                ).copy()
            ...
            raise ValueError("'distant_horizon_profile' must be scalar or a 1D or 2D array.")
        ...
        for i, row in enumerate(distant_horizon_profile):
            ...
            if row_str_mask.all():
                if not np.all(row == row[0]):
                    raise ValueError(f"distant_horizon_profile for placement #{i} contains multiple different strings.")
                str_mask[i] = True
            ...
            if row_num_mask.all():
                ...
                if not np.isfinite(numeric_row).all():
                    raise ValueError(f"distant_horizon_profile for placement #{i} contains NaN or infinite values.")
                num_mask[i] = True
                continue
        ...
        if distant_horizon_profile.shape[1] == 1:
            ...
            raise ValueError(
                f"Length of distant horizon profile sampling points ({distant_horizon_profile.shape[1]}) does not match the number of azimuths ({len(azimuths)})."
            )
        ...
        if np.any(_num_array < -3) or np.any(
            _num_array >= 90
        ):  # -3° is below even the most extreme angle that could occur on Mt. Everest
    ...
    def preprocess_hill_slope_and_azimuth(
        ...
        def _preprocess_data(varname, invals, fallback):
            ...
            for fp in np.unique(filepaths[~pd.isna(filepaths)]):
                if not isfile(fp):
                    raise FileNotFoundError(
                        f"{varname} contains string entries which must be existing {varname} rasters but cannot find file: {fp}"
                    )
                # generate a mask that applies also to the locations
                mask = filepaths == fp
                # extract a clipped sub raster covering the affected placements extent only to save time
                ...
                if clipped_raster is None:
                    ...
                    if np.isnan(iter_vals).any():
                        # if getting values fails, it could be because of interpolation method
                        # replace only those that failed by 'near' interpolation
                        iter_vals_near = gk.raster.interpolateValues(clipped_raster, self.locs[mask], mode="near")
                        iter_vals[np.isnan(iter_vals)] = iter_vals_near[np.isnan(iter_vals)]
                if np.isnan(iter_vals).any():
                    # we still have nans
                    if fallback is None:
                        ...
                        # if allowed, replace nans by fallback value
                        fallbacks_applied += np.isnan(iter_vals).sum()
                        iter_vals[np.isnan(iter_vals)] = (np.ones(shape=iter_vals.shape) * fallback)[
                            np.isnan(iter_vals)
                        ]
            ...
            if fallbacks_applied > 0:
                warnings.warn(
                    f"{varname} for {fallbacks_applied} out of {len(self.locs)} placements could not be extracted from provided {varname} files and has been replaced with fallback value {fallback}."
                )
        ...
        if north_slope is not None or east_slope is not None:
            if east_slope is None or north_slope is None:
                raise ValueError(
                    "When north_slope or east_slope is not None, east_slope and north_slope must both be provided."
                )
            ...
            general_slope = np.degrees(np.arctan(np.hypot(north_gradient, east_gradient)))

        elif general_slope is not None or downhill_azimuth is not None:
            if general_slope is None or downhill_azimuth is None:
                raise ValueError(
                    "When general_slope or downhill_azimuth is not None, general_slope and downhill_azimuth must both be provided."
                )
    ...
    def calculate_local_horizon_based_on_hillslope(
        ...
        if not (
            "general_slope" in self.plant_parameters_processed and "downhill_azimuth" in self.plant_parameters_processed
        ):
            raise AttributeError(
                "'general_slope' and 'downhill_azimuth' are required, run self.preprocess_hill_slope_and_azimuth() first.",
                flush=True,
            )
        ...
        if not np.isfinite(azimuthal_stepsize) or azimuthal_stepsize <= 0:
            raise ValueError("azimuthal_stepsize must be finite and greater than zero.")
        number_of_steps = 360.0 / azimuthal_stepsize
        if not np.isclose(number_of_steps, round(number_of_steps)):
            raise ValueError(f"azimuthal_stepsize={azimuthal_stepsize} does not divide 360 degrees evenly.")
        azimuths = np.arange(0, 360, azimuthal_stepsize)
    ...
    def preprocess_singleaxis_and_crossaxis(
        ...
        try:
            ...
            if "must not have None values" in str(e):
                raise ValueError(f"{e} This is required when tracking='singleaxis'.") from e
            raise
        ...
        if (not self._is_none(singleaxis_tilt).all()) and (not self._is_none(crossaxis_tilt).all()):
            ...
            if (
                self._is_none(self.plant_parameters_processed["general_slope"]).all()
                or self._is_none(self.plant_parameters_processed["downhill_azimuth"]).all()
            ):
                ...
                print(
                    "NOTE: No singleaxis_tilt/crossaxis_tilt information nor hill orientation (general_slope, downhill_azimuth) provided, flat terrain/axes will be assumed.",
                    flush=True,
                )
                ...
                singleaxis_tilt = np.degrees(np.arctan(np.tan(hill_slopes_rad) * np.cos(delta)))
    ...
    def permit_single_axis_tracking(self):
        ...
        with warnings.catch_warnings():
            ...
            for i, (_axazimuth, _axtilt, _caxtilt, _gcr, _backtrack, _max_angle) in enumerate(
                zip(
                ...
                if (tmp["surface_tilt"] < -1e-9).any():  # with numeric tolerance
                    raise ValueError("pvlib.tracking.singleaxis returned negative surface_tilt.")
    ...
    def estimate_plane_of_array_irradiances(self, transposition_model="perez", **kwargs):
        """
        ...
        **kwargs
        ...
        def _set_total_irradiance_per_side(front=True):
            ...
            bifaciality_mask = (
                np.ones(self.locs.count, dtype=bool)
                if front
                else (self.plant_parameters_processed["bifaciality_factor"] != 0)
            )

            _poa = pvlib.irradiance.get_total_irradiance(
                surface_tilt=(_modtilts if front else 180 - _modtilts)[..., bifaciality_mask],
                surface_azimuth=_modazimuths[..., bifaciality_mask]
                if front
                else ((_modazimuths + 180) % 360)[..., bifaciality_mask],
    ...
    def estimate_snow_coverage_loss(
        ...
        if not np.any(np.asarray(consider_snow_loss)):
            ...
            assert format in ["portrait", "landscape"], "format must be 'portrait' or 'landscape'."
            ...
            if (
                self.sim_data_full["snowfall_water_equivalent"].max() / 50 * 1000 * 100 <= threshold_snowfall
            ):  # m snowfall_water_equivalent is water equiv. in m/h x 100 cm/m, corrected by water over snow density
                ...
                self.sim_data["partial_snowcov"] = partial_snowcov[
                    self._time_sel_, :
                ]  # reduce to only daylight timesteps of interest
                return self
            ...
            def _format_input_to_locs_array(val, varname, assert_type=None):
                ...
                if _val.ndim == 0:  # scalar
                    ...
                    if not len(_val) == len(self.locs):
                        raise ValueError(
                            f"{varname} was provided as 1D array with length = {len(_val)} but does not match No. of locs: {len(self.locs)}"
                        )
                    return _val
                else:
                    raise ValueError(
                        f"Only scalar or 1D arrays are accepted for one-per-location arguments: {varname} = {val}"
                    )

            self_cleaning = _format_input_to_locs_array(
                val=self_cleaning, varname="self_cleaning", assert_type=bool
            )  # TODO make self_cleaning a plant_parameters_processed param like all others, allow to pass per location
            consider_snow_loss = _format_input_to_locs_array(
                val=consider_snow_loss, varname="consider_snow_loss", assert_type=bool
            )
            ...
            if self.tracking == "singleaxis":
                ...
                surface_tilt_full[np.ix_(self._time_sel_, ~self_cleaning)] = current_angles[:, ~self_cleaning]
            ...
            if np.any(np.abs(surface_tilt_full) < 10):
                warnings.warn(
                    f"Snow cover may occur on module tilt < 10°. note that only sliding and not melting effects are considered which may lead to unrealistically long snow cover duration."
                )
    ...
    def scale_to_unshaded_real_lra(self, max_scaling_factor: float = None):
        ...
        assert "direct_normal_irradiance" in self.sim_data, (
            "'direct_normal_irradiance' attribute is expected in self.sim_data"
        )
        assert "global_horizontal_irradiance" in self.sim_data, (
            "'global_horizontal_irradiance' attribute is expected in self.sim_data"
        )
        ...
        if not "distant_horizon_profile" in self.plant_parameters_processed:
            raise AttributeError(
                f"'distant_horizon_profile' is a mandatory attribute, run self.preprocess_distant_horizon_profile() or self.preprocess_horizon_profile() first."
            )
        ...
        if max_scaling_factor is not None:
            ...
            assert isinstance(max_scaling_factor, float) and max_scaling_factor >= 1, (
                "max_scaling_factor must be float >= 1.0 if given"
            )
        ...
        self.sim_data["global_horizontal_irradiance"] = self.sim_data["global_horizontal_irradiance"] + (
            _dni_flathorizon - self.sim_data["direct_normal_irradiance"]
        ) * np.cos(np.radians(self.sim_data["apparent_solar_zenith"]))
        ...
        for iloc in range(n_locations):
            ...
            for k, v in kwargs.items():
                ...
                # 2d input - must then match time x locs shape of sim data
                    ...
                    raise ValueError(f"Unknown input dimensionality ({arr.ndim}D) for kwarg '{k}'.")
            ...
            if self.tracking == "fixed":
                ...
                assert np.all(pvfts_args["surface_tilt"] == pvfts_args["surface_tilt"][0]), (
                    "surface_tilt must be constant over time for tracking = 'fixed'!"
                )
                assert np.all(pvfts_args["surface_azimuth"] == pvfts_args["surface_azimuth"][0]), (
                    "surface_azimuth must be constant over time for tracking = 'fixed'!"
                )
                ...
                if np.asarray(pvfts_args["axis_azimuth"], dtype=float).ndim == 0:
                    ...
                    assert np.all(
                        np.asarray(pvfts_args["axis_azimuth"], dtype=float)
                        == np.asarray(pvfts_args["axis_azimuth"], dtype=float)[0]
                ...
                assert np.all(np.abs(axis_diff) < 1e-9), (
            ...
            # 3D-VERSION
            if self.tracking == "singleaxis" and not (
                np.isclose(self.plant_parameters_processed["singleaxis_tilt"][iloc], 0)
                and np.isclose(self.plant_parameters_processed["crossaxis_tilt"][iloc], 0)
            ):
                ...
                if (
                    _root_term < -1e-12
                ):  # should not happen for a real terrain but axis and cross axis tilts might have been manipulated individually, so make sure
                    raise ValueError(
                        f"Inconsistent singleaxis_tilt ({np.degrees(_axis_tilt_rad)}°) and crossaxis_tilt ({np.degrees(_crossaxis_tilt_rad)}°) at location {iloc}."
                    )
                _root_term = max(
                    _root_term, 0.0
                )  # must be zero or positive for sqrt, fix potential floating point errors by max(x, 0)
                ...
                _downhill_gradient_east = _axis_gradient * np.sin(_axis_azimuth_rad) + _profile_gradient * np.cos(
                    _axis_azimuth_rad
                )
                _downhill_gradient_north = _axis_gradient * np.cos(_axis_azimuth_rad) - _profile_gradient * np.sin(
                    _axis_azimuth_rad
                )
            ...
            elif self.tracking == "fixed" and not np.isclose(
                self.plant_parameters_processed["general_slope"][iloc], 0.0
            ):
                ...
                _plant_downhill_azimuth_rad = np.radians(
                    self.plant_parameters_processed["downhill_azimuth"][iloc] % 360.0
                )
                ...
                _axis_tilt_rad = np.arctan(
                    np.tan(_plant_slope_rad) * np.cos(_axis_azimuth_rad - _plant_downhill_azimuth_rad)
                )
                # get the gradient in the crossaxis plane perpendicular to the main axis
                _crossaxis_tilt_rad = np.arctan2(
                    np.tan(_plant_slope_rad) * np.sin(_plant_downhill_azimuth_rad - _axis_azimuth_rad),
            ...
            if project:
                ...
                _terrain_normal = np.array(
                    [
                        np.sin(_plant_slope_rad)
                        * np.sin(
                            _plant_downhill_azimuth_rad
                        ),  # positive slope angle means horizontal components point in plant slope azimuth direction
                ...
                if not np.isclose(_rotation_angle_rad, _plant_slope_rad):
                    raise RuntimeError(
                        f"_plant_slope_rad ({_plant_slope_rad}) does not match the arccos of the dot product of the plant sloped plane and horizontal plane ({_rotation_angle_rad})!"
                    )
                ...
                def _rotate_angle_and_azimuth_to_level_plant_plane(
                    ...
                    vectors_flat = (
                        ...
                        + _rotation_axis
                        * np.sum(vectors * _rotation_axis, axis=-1, keepdims=True)
                        * (1.0 - np.cos(_rotation_angle_rad))
                    )
                    ...
                    inclination_flat_deg = np.degrees(
                        np.arctan2(vector_horizontal_flat, vector_up_flat)
                    )  # arctan2 is sign-true
                ...
                pvfts_args["solar_zenith"], pvfts_args["solar_azimuth"] = (
                    _rotate_angle_and_azimuth_to_level_plant_plane(
                        inclination_deg=pvfts_args["solar_zenith"],
                        azimuth_deg=pvfts_args["solar_azimuth"],
                    )
                )
                pvfts_args["surface_tilt"], pvfts_args["surface_azimuth"] = (
                    _rotate_angle_and_azimuth_to_level_plant_plane(
                ...
                _axis_inclination_flat, _axis_azimuth_flat = _rotate_angle_and_azimuth_to_level_plant_plane(
                    ...
                    + np.degrees(
                        _axis_tilt_rad
                    ),  # make sure we use an inclination angle from vertical for above transformation function, not simply the default axis tilt
                ...
                if not np.isclose(_axis_inclination_flat, 90.0, atol=1e-8):
                    raise RuntimeError(
                        f"Plant-plane leveling did not produce a horizontal row axis: {_axis_inclination_flat}° at location {iloc}."
                    )
                ...
                pvfts_args["gcr"] *= np.cos(
                    _crossaxis_tilt_rad
                )  # sloped row width increases over horizontal (defined) row width, so gcr is reduced
            ...
            assert (
                np.atleast_1d(pvfts_args["pvrow_height"])
                - 0.5
                * np.atleast_1d(pvfts_args["pvrow_width"])
                * np.sin(np.radians(np.max(pvfts_args["surface_tilt"])))
                > 0
            ).all(), (
    ...
    def configure_cec_module(
        ...
        if tracking not in ["fixed", "singleaxis"]:
            raise NotImplementedError(
                f"Unknown tracking value ({tracking}). Choose from: 'singleaxis' (HSAT), 'fixed' (tilt)"
            )
        self.tracking = tracking

        def _interpolate_module_params(projected_module, original_module_name, tech_year, start_year, database):
            if not isinstance(tech_year, int):
                raise TypeError("tech_year must be an integer when projected module is selected")
            ...
            try:
                ...
                if splitext(database)[-1] == "":
                    ...
                    raise TypeError(
                        f"database must be a csv file if not a known key in retrieve_sam(). Here: {database}"
                    )
        ...
        if isinstance(module, str):
            ...
            if module == "WINAICO WSx-240P6":
                ...
                try:
                    ...
                    raise RuntimeError(f"The module '{module}' is not in the CEC database.")
            else:
                raise TypeError(f"module must be str-formatted module name. Here: {module}")
        else:
            if tech_year is not None:
                print("NOTE: The tech_year argument is ignored when specific module parameters are given.")
        ...
        if "Length" not in module.index or pd.isnull(module["Length"]):
            ...
            try:
                ...
                warnings.warn(
                    f"module '{module.name}' from database has no Length information and no additional Length information found in RESKit dictionary. Add Length info manually in length_dict in configure_cec_module()"
                )
        if "Width" not in module.index or pd.isnull(module["Width"]):
            ...
            try:
                ...
                warnings.warn(
                    f"module '{module.name}' from database has no Width information and no additional Width information found in RESKit dictionary. Add Width info manually in width_dict in configure_cec_module()"
                )
        ...
        if match:
            ...
            raise ValueError(
                f"Invalid module_configuration format: '{module_configuration}'. Must be a string like '1P' or '3L'"
            )
    ...
    def simulate_with_interpolated_single_diode_approximation(
        self, consider_snow_cover: bool = False, format: str = "portrait", num_strings: int = None
    ):
        """
        ...
        if np.any(np.asarray(consider_snow_cover)):
            assert "partial_snowcov" in self.sim_data, (
                f"'partial_snowcov' must be calculated first if 'consider_snow_cover' is True for any location."
            )
        ...
        if not np.any(sel):
            ...
            if np.any(
                np.asarray(consider_snow_cover)
            ):  # TODO move this block to the plant (or module?) system setup and save parallel number of strings as attr
            ...
            for _string in range(num_strings):
                ...
                if not (self.plant_parameters_processed["bifaciality_factor"] == 0).all():
                    ...
                    poa_back = (
                        np.broadcast_to(self.plant_parameters_processed["bifaciality_factor"], sel.shape)[sel]
                        * self.sim_data["poa_backside_global"][sel]
                    )
                ...
                if (
                    num_strings > 1
                ):  # TODO power could be added iteratively for more than one string but how to deal with different voltages per string, how to combine into one MODULE_dc_voltage_at_mpp?
                    raise NotImplementedError(
                        "Define string-wise module_dc_power_at_mpp and module_dc_voltage_at_mpp combination for num_strings > 1 first."
                    )
        ...
        if (
            "capacity" in self.plant_parameters_processed
            and not self._is_none(self.plant_parameters_processed["capacity"]).any()
        ):
    ...
    def apply_inverter_losses(
        ...
        assert (
            "modules_per_string" in self.plant_parameters_processed
        )  # TODO #ISSUE285 write preprocessing functions to add modules per string and strings per inverter
        assert "strings_per_inverter" in self.plant_parameters_processed
        assert "capacity" not in self.plant_parameters_processed, (
            "Cannot simultaneously provide 'capacity' and inverter-string parameters"
        )
