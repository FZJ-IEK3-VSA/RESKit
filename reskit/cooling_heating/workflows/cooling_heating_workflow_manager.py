...
class CoolingHeatingWorkflowManager(WorkflowManager):
    def __init__(self, placements):
        ...
        assert all([a in placements.columns for a in ["lon","lat","capacity"]]), "Placements must contain the columns lon,lat and capacity"
        super().__init__(placements)

        #Set thermodynamic data [1] for air density, [2] for air heat capacity
        airData = np.array([[0.0002793, 0.0002793, 0.0002792, 0.0002792, 0.0002793, 0.0002794, 0.0002795, 0.0002796, 0.0002798,0.0002799, 0.0002802, 0.0002804], #heat capacity cp in kWh/(kgK)
    ...
    def calculate_fan_power(self, temperatureCoolant, heatTransferDelta=5, efficiencyFan=0.7, pressureDropAir=261, designTemperature=None):
        ...
        WFan = (1 / (cpAir * (temperatureCoolant - heatTransferDelta - airTemp) * densityAir)) / efficiencyFan * pressureDropAir
        WFan = WFan / 1000 / 3600  # Convert J to kWh
        if designTemperature:
            return -WFan
        else:
            WFan[(temperatureCoolant - heatTransferDelta - airTemp) <= 0] = np.inf  # Assign high value if cooling is not possible
    ...
    def calculate_pump_power(self, temperatureCoolant, heatTransferDelta=5, efficiencyPump=0.7, pressureDropWater=200000, designTemperature=None):
        ...
        #Calculate Power demand for 1 kWh of cooling:
        WPump = (1 / (cp * (temperatureCoolant - heatTransferDelta - airTemp) * density)) / efficiencyPump * pressureDropWater
