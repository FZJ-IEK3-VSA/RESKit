...
class EGS_workflowmanager:
    ...
    def loadData(self, vars:list, source:str, newVarNamesDict:dict=None):
        ...
        def set_dims(**vals):
            ...
            for name in vals:
                ...
                if hasattr(self, name):
                    ...
                    assert np.allclose(self.__getattribute__(name), value), f'dimensions do not align, pls check dim: {name} in file: {source}'
                else:
        ...
        set_dims(
            latsGlobal=ds.lat.values,
            lonsGlobal=ds.lon.values
        )
        ...
        for var in vars:
            if 'depth' in ds.dims:
                ...
                ds_flat = ds
            data_var_depth = self._EGS_NC4_to_raster(
                nc4_obj=ds_flat,
                varname=var
            )
    ...
    def __calculatePlacementHeat(self):
        ...
        if 'temperature' in variables: variables.remove('temperature')
    ...
    def VolumeMethod(self):
        ...
        #total enthalpy
        Enth = rho_rock * cp_rock * self.data['reservoir_size_m3'] * (self.sim_data['temperature'] - self.placements['surface_temperature'].values)
        ...
        self.sim_data_VM['T_Rock_abandon_VM_degC'] = self.sim_data['temperature'] - self.sim_data_VM['dT_total_res_VM_K']
    ...
    def GringartenMethodFixedT(self):
        ...
        q = np.sqrt((t_D*k_R*rho_rock*cp_rock)/((self.rho_water*self.cp_water)**2 *t)) * y * z #m^3/s?, inj. water volume flow per fracture, see Augustine2016 eq(2)
    ...
    def calculatePumpLosses(self, method='default', techMethod=None):
        ...
        Vdot_m3_per_s_per_well = 1/self.rho_water * sim_data_techmethod[f'mdot_water_{tech_method_short}_kg_per_s_per_well']
        ...
        sim_data_techmethod[f'P_out_net_{tech_method_short}_MW'] = sim_data_techmethod[f'P_out_{tech_method_short}_MW'] - P_pump_MW
    ...
    def calculateCosts(self, method='default', techMethod=None):
        ...
        if method == 'aghahosseini2020':
            ...
            #add up
            CAPEX_Total_MUSD = CAPEX_Well_MUSD * self.data['N_wells'] \
                    + CAPEX_Plant_MUSD \
                    + CAPEX_Stim_MUSD + CAPEX_Distr_MUSD + CAPEX_Expl_MUSD
            ...
            CAPEX_Well_MUSD_SAM_Intermed1 = (1326526.313 + 293.4517365 * self.depths + 0.189267288 * self.depths**2) / 1E6
            ...
            # Other
            CAPEX_Stim_MUSD = 2.5
            CAPEX_Pump_MUSD = 1720 * sim_data_techmethod[f'P_Pump_{tech_method_short}_MW'] * self.data['n_production_wells_1'] / 1E3
            CAPEX_Expl_MUSD = 1.85 
            # add up
            CAPEX_Total_MUSD = CAPEX_Well_MUSD * self.data['N_wells'] \
                    + CAPEX_Plant_MUSD \
                    + CAPEX_Stim_MUSD + CAPEX_Pump_MUSD + CAPEX_Expl_MUSD
        else:
            CAPEX_Total_MUSD = np.nan
        
        #calc annuity
        self.data['annuity'] = ((1 + self.data['WACC'])**self.data['lifetime_a'] * self.data['WACC']) / ((1 + self.data['WACC'])**self.data['lifetime_a'] - 1)
    ...
    def calculateLCOE(self, techMethod):
        ...
        sim_data_techmethod[f'LCOE_gross_{tech_method_short}_EUR_per_kWh'] = \
            sim_data_techmethod[f'TOTEX_MUSD_{tech_method_short}_per_a'] * self.USD2EUR * 1E6 / (sim_data_techmethod[f'P_out_{tech_method_short}_MW'] * 1E3 * 8760) 
        sim_data_techmethod[f'LCOE_{tech_method_short}_EUR_per_kWh'] = \
            sim_data_techmethod[f'TOTEX_MUSD_{tech_method_short}_per_a'] * self.USD2EUR * 1E6 / (sim_data_techmethod[f'P_out_net_{tech_method_short}_MW'] * 1E3 * 8760)
    ...
    def getRegenerationTime(self, techMethod):
        ...
        Q_out_Wa = sim_data_techmethod[f'Qdot_out_{tech_method_short}_MW'] * 1E6 * self.data['lifetime_a'] #Wa = Watt*year
        delta_depth = self.depths[1] - self.depths[0] #m
        regeneration_time = Q_out_Wa / (self.data['reservoir_size_m3'] / delta_depth * self.placements['qdot_sust_W_per_m2'].values) #a
    ...
    def saveOutput(self, savepath=None, deepsave=False):
        ...
        def _convertToXr(placements):
            ...
            ds_out = xr.Dataset(
                coords={
                    ...
                    attrs=dict(description=f"RESkit simulation results for EGS.", timestamp = f" Generated at: {datetime.now()} "),
            )
        ...
        if savepath is None:
            ...
            if filetype.lower() == '.shp':
                #do shapefile
                self.placements['geom'] = self.placements[['lon', 'lat']].apply(lambda x: gk.geom.point(x[0], x[1]), axis=1)
    ...
    @staticmethod
    def volumeMethodSingular(reservoir_size_m3, dT_drawdown, recovery_factor, temperature, surface_temperature=10, lifetime_a=30, depth=3000, n_wells=2, T_inj=80):
        '''calculate the enthalpy from the temperature
        '''
        ...
        CAPEX_Total_MUSD = CAPEX_Well_MUSD * n_wells \
                + CAPEX_Plant_MUSD \
                + CAPEX_Stim_MUSD + CAPEX_Distr_MUSD + CAPEX_Expl_MUSD
