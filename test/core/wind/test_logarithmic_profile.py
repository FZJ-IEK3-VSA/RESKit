...
def test_roughness_from_levels():
    ...
    r = roughness_from_levels(low_wind_speed=np.array([3, 4, 5]), low_height=10, high_wind_speed=np.array([4, 5, 6]), high_height=50)
...
def test_roughness_from_clc():
    ...
    r = roughness_from_clc(clc_path=TEST_DATA["clc-aachen_clipped.tif"], loc=[loc1, loc2, loc3], window_range=2)
