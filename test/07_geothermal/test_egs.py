...
def placements():
    ...
    geoms = [
        gk.geom.point(lon, lat, srs=gk.srs.loadSRS(4326))
...
def test_output_length(placements, egs_output):
    ...
    assert len(egs_output.placements) == len(placements), (
        "Mismatch in number of placements."
    )
...
def test_lcoe_vm(egs_output):
    ...
    assert allclose(egs_output.LCOE_VM_EUR_per_kWh, expected), (
        "LCOE_VM values do not match!"
    )
...
def test_lcoe_gr(egs_output):
    ...
    assert allclose(egs_output.LCOE_GR_EUR_per_kWh, expected), (
        "LCOE_GR values do not match!"
    )
...
def test_lcoe_su(egs_output):
    ...
    assert allclose(egs_output.LCOE_SU_EUR_per_kWh, expected), (
        "LCOE_SU values do not match!"
    )
