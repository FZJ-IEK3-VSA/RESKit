...
def test_WorkflowManager_spatial_disagregation(
    ...
    man.read(
        variables=["global_horizontal_irradiance", "direct_horizontal_irradiance",],
        source_type="ERA5",
        source=rk.TEST_DATA["era5-like"],
        set_time_index=True,
        verbose=False,
...
def test_WorkflowManager_to_xarray(
    ...
    xds = man.to_xarray(
        output_netcdf_path=None, output_variables=None, _intermediate_dict=False,
    )
...
def test_WorkflowQueue():
    # Create a queue
    queue = WorkflowQueue(workflow=simple_workflow, era5_path=TEST_DATA["era5-like"],)

    # append jobs to queue
    placements = pd.read_csv(TEST_DATA["turbine_placements.csv"])

    queue.append(
        key="run_1", placements=placements.iloc[::2, :], var1=0.5, var2="pants",
    )

    queue.append(
        key="run_2", placements=placements.iloc[1::3, :], var1=0.75, var2="cats",
    )

    queue.append(
        key="run_3", placements=placements.iloc[::4, :], var1=0.5, var2="rectangle",
    )

    queue.append(
        key="run_4", placements=placements.iloc[1::5, :], var1=0.75, var2="plane",
    )
