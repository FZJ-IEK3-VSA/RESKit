...
def test_openfield_pv_iconlam(pt_pv_placements_Zimbabwe):
    ...
    assert np.isclose(float(gen["lon"].fillna(0).mean()), 28.517050471694553)
    assert np.isclose(float(gen["lat"].fillna(0).mean()), -20.116783940337438)
    assert np.isclose(float(gen["tilt"].fillna(0).mean()), 20.518096540981862)
    assert np.isclose(float(gen["azimuth"].fillna(0).mean()), 0.0)
    assert np.isclose(float(gen["elev"].fillna(0).mean()), 300.0)
    assert np.isclose(float(gen["global_horizontal_irradiance"].fillna(0).mean()), 345.8985498434407)
    assert np.isclose(float(gen["direct_horizontal_irradiance"].fillna(0).mean()), 280.1782227565057)
    assert np.isclose(float(gen["surface_wind_speed"].fillna(0).mean()), 1.5444383549801568)
    assert np.isclose(float(gen["surface_pressure"].fillna(0).mean()), 51466.08737549783)
    assert np.isclose(float(gen["surface_air_temperature"].fillna(0).mean()), 15.758056408513792)
    assert np.isclose(float(gen["surface_dew_temperature"].fillna(0).mean()), 8.828181440758382)
    assert np.isclose(float(gen["solar_azimuth"].fillna(0).mean()), 91.82370031707131)
    assert np.isclose(float(gen["apparent_solar_zenith"].fillna(0).mean()), 28.175968348157248)
    assert np.isclose(float(gen["direct_normal_irradiance"].fillna(0).mean()), 358.36529124792463)
    assert np.isclose(float(gen["extra_terrestrial_irradiance"].fillna(0).mean()), 823.8760486889863)
    assert np.isclose(float(gen["air_mass"].fillna(0).mean()), 2.531281971257186)
    assert np.isclose(float(gen["diffuse_horizontal_irradiance"].fillna(0).mean()), 66.19574793171348)
    assert np.isclose(float(gen["angle_of_incidence"].fillna(0).mean()), 28.08243771460588)
    assert np.isclose(float(gen["poa_global"].fillna(0).mean()), 343.5207486460009)
    assert np.isclose(float(gen["poa_direct"].fillna(0).mean()), 276.87045606287734)
    assert np.isclose(float(gen["poa_diffuse"].fillna(0).mean()), 66.65029258312369)
    assert np.isclose(float(gen["poa_sky_diffuse"].fillna(0).mean()), 64.84121783364569)
    assert np.isclose(float(gen["poa_ground_diffuse"].fillna(0).mean()), 1.809074749477973)
    assert np.isclose(float(gen["cell_temperature"].fillna(0).mean()), 25.88995881968958)
    assert np.isclose(float(gen["module_dc_power_at_mpp"].fillna(0).mean()), 94.04783947529539)
    assert np.isclose(float(gen["module_dc_voltage_at_mpp"].fillna(0).mean()), 18.76166349286728)
    assert np.isclose(float(gen["capacity_factor"].fillna(0).mean()), 0.3493657053955156)
    assert np.isclose(float(gen["total_system_generation"].fillna(0).mean()), 5083.137258197374)
...
def test_openfield_pv_merra_ryberg2019(pt_pv_placements):
    ...
    assert np.isclose(float(gen["module_dc_power_at_mpp"].fillna(0).mean()), 12.625213000934778)
...
def test_openfield_pv_sarah_unvalidated(pt_pv_placements):
    ...
    assert np.isclose(float(gen["diffuse_horizontal_irradiance"].fillna(0).mean()), 16.019586491017332)
    assert np.isclose(float(gen["angle_of_incidence"].fillna(0).mean()), 18.703054670614844)
    ...
    assert np.isclose(float(gen["poa_sky_diffuse"].fillna(0).mean()), 27.582410926737612)
    assert np.isclose(float(gen["poa_ground_diffuse"].fillna(0).mean()), 1.180438286074874)
    assert np.isclose(float(gen["cell_temperature"].fillna(0).mean()), 4.738007290882006)
    assert np.isclose(float(gen["module_dc_power_at_mpp"].fillna(0).mean()), 45.17960123200836)
    assert np.isclose(float(gen["module_dc_voltage_at_mpp"].fillna(0).mean()), 11.939309430641291)
    assert np.isclose(float(gen["capacity_factor"].fillna(0).mean()), 0.15035309405307454)
    assert np.isclose(float(gen["total_system_generation"].fillna(0).mean()), 300.70618810614906)
