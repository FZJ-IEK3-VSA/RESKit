...
def test_openfield_pv_era5(pt_pv_placements):
    ...
    assert np.isclose(
        float(gen['solar_azimuth'].fillna(0).mean()), 68.6008947378997)
    assert np.isclose(
    ...
    assert np.isclose(
        float(gen['air_mass'].fillna(0).mean()), 3.9097505883982726)
    assert np.isclose(float(gen['diffuse_horizontal_irradiance'].fillna(
        0).mean()), 20.956699894906208)
    assert np.isclose(
        float(gen['angle_of_incidence'].fillna(0).mean()), 19.148843582996513)
    assert np.isclose(
        float(gen['poa_global'].fillna(0).mean()), 67.12122204692297)
    assert np.isclose(
        float(gen['poa_direct'].fillna(0).mean()), 37.85854851365722)
    assert np.isclose(
        float(gen['poa_diffuse'].fillna(0).mean()), 29.262673533265747)
    assert np.isclose(
...
def test_openfield_pv_merra_ryberg2019(pt_pv_placements):
    ...
    assert np.isclose(
        float(gen['solar_azimuth'].fillna(0).mean()), 67.69226199649943)
    ...
    assert np.isclose(
        float(gen['air_mass'].fillna(0).mean()), 3.9380521870930165)
    ...
    assert np.isclose(float(gen['diffuse_horizontal_irradiance'].fillna(
        0).mean()), 19.589003059825554)
    print(float(gen['angle_of_incidence'].fillna(0).mean()))
    assert np.isclose(
        float(gen['angle_of_incidence'].fillna(0).mean()), 18.915157478877003)
    assert np.isclose(
        float(gen['poa_global'].fillna(0).mean()), 38.581645380369565)
    assert np.isclose(
        float(gen['poa_direct'].fillna(0).mean()), 15.491403834596987)
    assert np.isclose(
        float(gen['poa_diffuse'].fillna(0).mean()), 23.090241545772578)
    assert np.isclose(
        float(gen['poa_sky_diffuse'].fillna(0).mean()), 22.5137080215302)
    assert np.isclose(
...
def test_openfield_pv_sarah_unvalidated(pt_pv_placements):
    ...
    assert np.isclose(
        float(gen['solar_azimuth'].fillna(0).mean()), 68.01052582121625)
    assert np.isclose(
    ...
    assert np.isclose(
        float(gen['air_mass'].fillna(0).mean()), 3.7818783820335424)
    assert np.isclose(float(gen['diffuse_horizontal_irradiance'].fillna(
        0).mean()), 16.019586491017332)
    assert np.isclose(
        float(gen['angle_of_incidence'].fillna(0).mean()), 18.703054670614844)
    assert np.isclose(
        float(gen['poa_global'].fillna(0).mean()), 140.8677773815084)
    assert np.isclose(
        float(gen['poa_direct'].fillna(0).mean()), 112.10492816869589)
    assert np.isclose(
        float(gen['poa_diffuse'].fillna(0).mean()), 28.762849212812483)
    assert np.isclose(
