...
def test_openfield_pv_iconlam(pt_pv_placements_Zimbabwe):
    ...
    assert gen["modtilt"].shape == (483,)
    assert gen["modazimuth"].shape == (483,)
    ...
    assert np.isclose(float(gen["modtilt"].fillna(0).mean()), 20.518451664897075)
    assert np.isclose(float(gen["modazimuth"].fillna(0).mean()), 0.0)
    ...
    assert np.isclose(float(gen["surface_wind_speed"].fillna(0).mean()), 1.5562901237076703)
    assert np.isclose(float(gen["surface_pressure"].fillna(0).mean()), 50863.302503769664)
    assert np.isclose(float(gen["surface_air_temperature"].fillna(0).mean()), 15.628519565010992)
    assert np.isclose(float(gen["surface_dew_temperature"].fillna(0).mean()), 8.733685869278785)
    assert np.isclose(float(gen["solar_azimuth"].fillna(0).mean()), 91.13197502945316)
    assert np.isclose(float(gen["apparent_solar_zenith"].fillna(0).mean()), 27.516630295133602)
    assert np.isclose(float(gen["direct_normal_irradiance"].fillna(0).mean()), 357.86282482022784)
    assert np.isclose(float(gen["extra_terrestrial_irradiance"].fillna(0).mean()), 814.181544671707)
    ...
    assert np.isclose(float(gen["angle_of_incidence"].fillna(0).mean()), 27.3999712014697)
    ...
    assert np.isclose(float(gen["cell_temperature"].fillna(0).mean()), 25.73411503556805)
    assert np.isclose(float(gen["module_dc_power_at_mpp"].fillna(0).mean()), 94.00160119470556)
    assert np.isclose(float(gen["module_dc_voltage_at_mpp"].fillna(0).mean()), 18.77611256028308)
...
def test_openfield_pv_era5(pt_pv_placements):
    ...
    assert gen["modtilt"].shape == (560,)
    assert gen["modazimuth"].shape == (560,)
    ...
    assert np.isclose(float(gen["modtilt"].fillna(0).mean()), 39.19976325987092)
    assert np.isclose(float(gen["modazimuth"].fillna(0).mean()), 180.0)
    assert np.isclose(float(gen["elev"].fillna(0).mean()), 300.0)
    ...
    assert np.isclose(float(gen["module_dc_voltage_at_mpp"].fillna(0).mean()), 14.349977855519542)
...
def test_openfield_pv_merra_ryberg2019(pt_pv_placements):
    ...
    assert np.isclose(float(gen["lat"].fillna(0).mean()), 50.80320853112445)
    print(float(gen["modtilt"].fillna(0).mean()))
    assert np.isclose(float(gen["modtilt"].fillna(0).mean()), 39.19976325987092)
    print(float(gen["modazimuth"].fillna(0).mean()))
    assert np.isclose(float(gen["modazimuth"].fillna(0).mean()), 180.0)
...
def test_openfield_pv_sarah_unvalidated(pt_pv_placements):
    ...
    assert np.isclose(float(gen["module_dc_voltage_at_mpp"].fillna(0).mean()), 11.940731512724874)
