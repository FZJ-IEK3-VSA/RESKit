...
def pt_pv_placements() -> pd.DataFrame:
    ...
    # Longitude  # [ -6.8, -6.8, -6.8]
    placements['lon'] = [-6.8, -6.8, -6.8]
    # Latitude  # [ 31.0, 31.4, 31.0,]
    placements['lat'] = [31.0, 31.4, 31.0, ]
    placements['area_m2'] = [1E6, 5E6, 6E6]
    repeats = int(3/3)
    placements = placements.loc[placements.index.repeat(
        repeats)].reset_index(drop=True)
    return placements
...
def test_CSP_PTR_ERA5(pt_pv_placements):
    ...
    assert np.allclose(out["LRA_factor_direct_normal_irradiance"].values, [
    ...
    assert np.isclose(
        out['direct_normal_irradiance'].values.mean(), 278.73085651103776)
    assert np.isclose(
        out['direct_normal_irradiance'].values.std(), 332.3526428406074)
    assert np.isclose(out['direct_normal_irradiance'].values.min(), 0.0)
    assert np.isclose(
        out['direct_normal_irradiance'].values.max(), 982.451222209881)
    ...
    assert np.isclose(
        out['Power_net_total_per_day_Wh'].values.std(), 797437011.9432147)
    assert np.isclose(out['Power_net_total_per_day_Wh'].values.min(), 0.0)
    assert np.isclose(
        out['Power_net_total_per_day_Wh'].values.max(), 2560268088.640056)
