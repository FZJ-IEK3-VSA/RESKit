...
def test_applyHTFHeatLossModel(pt_PTRWorkflowManager_heat_loss):
    ...
    assert np.isclose(wfm.sim_data["HTF_mean_temperature_C"].mean(), 133.1105765337709)
    assert np.isclose(wfm.sim_data["HTF_mean_temperature_C"].std(), 97.85535073399551)
...
def test_get_totex(pt_PTRWorkflowManager_initialized):
    ...
    TOTEX_EUR_per_a = wfm._get_totex(CAPEX_total_EUR_per_a=10, OPEX_EUR_per_a=2,)
...
def test_get_totex_from_self(pt_PTRWorkflowManager_economics):
    ...
    ).all()  # use those values, if 'Parasitics_solarfield_W_el' are bought as varOPEX
...
def pt_PTRWorkflowManager_parasitics() -> PTRWorkflowManager:
    ...
    wfm.sim_data["HeattoHTF_W"] = (
        wfm.sim_data["HeattoPlant_W"] * 1e9
    )  # do this big, as only the +- sign should be relevant
    return wfm
...
def test_calculateParasitics(pt_PTRWorkflowManager_parasitics):
    ...
    wfm.calculateParasitics(
        calculationmethod="dersch2018", params=params_dersch,
    )
