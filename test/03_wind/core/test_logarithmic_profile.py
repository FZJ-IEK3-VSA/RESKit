...
def test_roughness_from_levels():
    ...
    r = roughness_from_levels(low_wind_speed=np.array(
...
def test_roughness_from_clc():
    ...
    r = roughness_from_clc(
        clc_path=TEST_DATA["clc-aachen_clipped.tif"], loc=loc1)
    ...
    r = roughness_from_clc(
        clc_path=TEST_DATA["clc-aachen_clipped.tif"], loc=[loc1, loc2, loc3], window_range=2)
...
def test_roughness_from_land_cover_source():
    ...
    r = roughness_from_land_cover_source(source=TEST_DATA["ESA_CCI_2018_clip.tif"], loc=[
                                         loc1, loc2, loc3], land_cover_type="cci")
    assert np.isclose(r[0], 0.75)
