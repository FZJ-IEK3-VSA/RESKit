...
def test_defaultPaths ():
    assert DEFAULT_PATHS['waterdepthFile'] is not None, "waterdepthFile should not be None"
    assert DEFAULT_PATHS['distancetoCoast'] is not None, "waterdepthFile should not be None"
...
def test_distanceToCoastLine():
    ...
    assert np.isclose(calcuated_distance,23), 'the distance-to-coatslibe-calculation does not work properly'

def test_waterDepthFromLocation():
    waterdepth_exact=22
    depth= waterDepthFromLocation(lat,
                                lon,
                                waterDepthFolderPath=DEFAULT_PATHS.get("waterdepthFile")
                                )
    assert np.isclose(depth, waterdepth_exact), 'the distanceToCoatsLineFile is not working correct'
...
def test_getRatedCostFromWaterDepth():
    ...
    assert getRatedCostFromWaterDepth(17)==getRatedCostFromWaterDepth(-17), 'negative avlues are handled incorrect'
